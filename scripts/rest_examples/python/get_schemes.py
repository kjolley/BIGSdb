...
def main():
    base_uri = "https://rest.pubmlst.org"
...
def get_matching_schemes(db):
    if re.search(r"definitions", db["description"], flags=0):
        ...
        if not "schemes" in db_attributes.keys():
            return
        schemes = requests.get(db_attributes["schemes"]).json()
        for scheme in schemes["schemes"]:
            if args.match:
                if not re.search(args.match, scheme["description"], flags=0):
                    continue
            if args.exclude:
                if re.search(args.exclude, scheme["description"], flags=0):
                    continue
            output = "%s\t%s\t%s" % (
                db["description"],
                scheme["description"],
