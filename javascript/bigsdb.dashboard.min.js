...
$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updatePrefs=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(){reloadAllElements(d)})});$("#add_element").click(function(){var a=getNextid();addElement(d,a)});$("div#dashboard").on("click",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(d,a)});$("div#dashboard").on("click",
".setup_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click",".dashboard_explore_element",function(){var a=$(this).attr("data-id");if(elements[a].url){var b=elements[a].url,c={};elements[a].post_data&&(c=elements[a].post_data);c.sent=1;b.includes("&page=query")&&$("#include_old_versions").prop("checked")&&(c.include_old="on");post(elements[a].url,c)}});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=
$(this).attr("id").replace("_"+a,"");changeElementDimension(d,b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");changeElementAttribute(d,b,a,$(this).val())});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updatePrefs=1&attribute=default&value=0",function(){window.location=url})})});
...
function changeElementAttribute(a,b,c,d){elements[b][c]!==d&&("specific_values"!==c||Array.isArray(d)||(d=d.includes("\n")?d.split("\n"):d.split()),d=clean_value(d),elements[b][c]=d,currentRequest=$.ajax({url:url,type:"POST",data:{db:instance,page:"dashboard",updatePrefs:1,attribute:"elements",value:JSON.stringify(elements)},beforeSend:function(){null!=currentRequest&&currentRequest.abort()},success:function(){reloadElement(a,b)}}))}
function applyFormatting(){fitty(".dashboard_big_number",{maxSize:64});$(".item-content div.subtitle a").tooltip()}function getNextid(){return 0===Object.keys(elements).length?1:Math.max.apply(Math,[].concat($jscomp.arrayFromIterable(Object.keys(elements))))+1}
...
function editElement(a,b,c){$("span#control_"+b).hide();$("span#wait_"+b).show();$.get(url+"&page=dashboard&control="+b,function(c){$(c).appendTo("body").modal();$("#edit_elements").prop("checked")&&$("span#control_"+b).show();$("span#wait_"+b).hide();show_or_hide_control_elements(a,b);$("select.watermark_selector").fontIconPicker({theme:"fip-darkgrey",emptyIconValue:"none"});$("div.modal").on("change","#"+b+"_visualisation_type",function(){show_or_hide_control_elements(a,b);check_and_show_visualisation(a,
b)});$("div.modal").on("change","#"+b+"_breakdown_display,#"+b+"_specific_value_display,#"+b+"_specific_values",function(){show_or_hide_control_elements(a,b);check_and_show_visualisation(a,b)});$("div.modal").on($.modal.AFTER_CLOSE,function(a,b){$("div.modal").remove()})})}
function show_or_hide_control_elements(a,b){a=$("input[name='"+b+"_visualisation_type']:checked").val();b=$("#"+b+"_specific_value_display").val();$("li#value_selector").css("display","breakdown"===a?"none":"block");$("li#breakdown_display_selector").css("display","breakdown"===a?"block":"none");$("li#specific_value_display_selector").css("display","breakdown"===a?"none":"block");"specific values"===a&&($("fieldset#change_duration_control").css("display","number"===b?"inline":"none"),$("fieldset#design_control").css("display",
"number"===b?"inline":"none"))}function check_and_show_visualisation(a,b){var c=$("input[name='"+b+"_visualisation_type']:checked").val();$("#"+b+"_breakdown_display").val();var d=$("#"+b+"_specific_value_display").val(),e=$("#"+b+"_specific_values").val();"specific values"===c&&("0"!=d&&0!=e.length?changeElementAttribute(a,b,"display","field"):changeElementAttribute(a,b,"display","setup"))}
function reloadElement(a,b){$.get(url+"&page=dashboard&element="+b,function(a){try{$("div#element_"+b+"> .item-content > .ajax_content").html(JSON.parse(a).html),elements[b]=JSON.parse(a).element,applyFormatting()}catch(d){console.log(d.message)}})}function reloadAllElements(a){$.each(Object.keys(elements),function(b,c){reloadElement(a,c)})}
...
function changeElementDimension(a,b,c){var d=$("div.item[data-id='"+b+"'] > div.item-content"),e=d.attr("class").split(/\s+/);$.each(e,function(a,b){b.includes("dashboard_element_"+c)&&d.removeClass(b)});e=$("input[name='"+b+"_"+c+"']:checked").val();d.addClass("dashboard_element_"+c+e);$("span#"+b+"_"+c).html(e);elements[b][c]=Number(e);$.ajax({url:url,type:"POST",data:{db:instance,page:"dashboard",updatePrefs:1,attribute:"elements",value:JSON.stringify(elements)},success:function(){reloadElement(a,
b)}});a.refreshItems().layout()}function saveElements(a){$.post(url,{db:instance,page:"dashboard",updatePrefs:1,attribute:"elements",value:JSON.stringify(elements)});saveLayout(a)}function serializeLayout(a){a=a.getItems().map(function(a){return a.getElement().getAttribute("data-id")});return JSON.stringify(a)}
...
function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(url+"&resetDefaults=1",function(){$("#layout").val("left-top");$("#fill_gaps").prop("checked",!0);$("#enable_drag").prop("checked",!1);$("#edit_elements").prop("checked",!1);$("#remove_elements").prop("checked",!1);location.reload()})};
