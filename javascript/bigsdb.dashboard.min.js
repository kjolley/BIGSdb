...
"");b=b.replace("_"+a,"");changeElementAttribute(c,b,a,$(this).val())});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger","_panel");"none"==$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+b).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",
function(){$.get(url+"&page=dashboard&updatePrefs=1&attribute=default&value=0",function(){window.location=url})})});function post(a,b){var c=$("<form></form>");c.attr("method","post");c.attr("action",a);$.each(b,function(a,b){var d=$("<input></input>");d.attr("type","hidden");d.attr("name",a);d.attr("value",b);c.append(d)});$(document.body).append(c);c.submit()}
