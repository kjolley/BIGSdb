...
function addElement(a,b){0===Object.keys(elements).length&&$("div#empty").html("");var c=url+"&page=dashboard&new="+b,d=$("#add_field").val();d&&(c+="&field="+d);lastAjaxUpdate=(new Date).getTime();$.get(c,function(c){try{var d=document.createRange().createContextualFragment(JSON.parse(c).html);b in elements||(a.add([d.firstChild]),elements[b]=JSON.parse(c).element,saveElements(a));applyFormatting();$("div#element_"+b+" div.item-content").css("visibility","visible")}catch(g){console.log(g.message)}})}
...
function saveLayout(a){var b=0;(new Date).getTime()-lastAjaxUpdate<ajaxDelay&&(b=ajaxDelay);setTimeout(function(){lastAjaxUpdate=(new Date).getTime();var b=serializeLayout(a);$.post(url,{db:instance,page:"dashboard",updateDashboard:1,attribute:"order",value:b})},b)}function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(url+"&resetDefaults=1",function(){location.reload()})}function createNew(){$.get(url+"&newDashboard=1",function(){location.reload()})}
