...
$(function(){showOrHideElements();$("select#add_field,label[for='add_field']").css("display","inline");var a=$("#fill_gaps").prop("checked");try{var c=(new Muuri("#dashboard",{dragEnabled:!0,layout:{fillGaps:a},dragStartPredicate:function(a,c){return enable_drag}})).on("move",function(){setGridMargins(c);saveLayout(c)});order&&loadLayout(c,order)}catch(d){console.log(d.message)}$("#record_age_slider").slider({min:0,max:7,value:recordAge,slide:function(a,c){$("#record_age").html(recordAgeLabels[c.value])},
...
{direction:"right"},"fast",function(){$("#modify_dashboard_panel").is(":visible")?$("#modal_overlay").addClass("open"):$("#modal_overlay").removeClass("open")});$("#dashboard_panel_trigger").show();return!1});$("#dashboard_panel_trigger").show();$("#modal_overlay").on("click",function(){$("#modify_dashboard_panel").hide();$(this).removeClass("open")});$("#fill_gaps").change(function(){a=$("#fill_gaps").prop("checked");try{c._settings.layout.fillGaps=a,c.layout()}catch(e){}var d=url+"&page=dashboard&updateDashboard=1&type="+
dashboard_type;"undefined"!==typeof projectId&&(d+="&project_id="+projectId);d+="&attribute=fill_gaps&value="+(a?1:0);$.ajax({url:d}).done(function(a){updateDashboardName(JSON.parse(a).dashboard_name)})});$("#enable_drag").change(function(){enable_drag=$("#enable_drag").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=enable_drag&value="+(enable_drag?1:0)})});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=edit_elements&value="+
(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#open_new").change(function(){var a=$("#open_new").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(a?1:0)}).done(function(){reloadElementsWithURL()})});
$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked"),c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=include_old_versions&value="+(a?1:0))}).done(function(c){updateDashboardName(JSON.parse(c).dashboard_name);$("#filter_versions").html(a?"all":"current");reloadAllElements();bumpVersion()})});$("#dashboard_palette").change(function(){var a=$("#dashboard_palette").val(),
c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=palette&value="+a)}).done(function(a){updateDashboardName(JSON.parse(a).dashboard_name);bumpVersion();reloadAllElements()})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val(),c=url+"&page=dashboard&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);c+="&updateDashboardName="+encodeURIComponent(a);
a.length&&$.ajax(c)});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();a=url+"&page=dashboard&setActiveDashboard="+a+"&type="+dashboard_type;"undefined"!==typeof projectId&&(a+="&project_id="+projectId);$.ajax({url:a}).done(function(){$("input#search").length?$("input#search").trigger("click"):location.reload()})});$("#add_element").click(function(){var a=getNextid();addElement(c,a);setGridMargins(c)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=
$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_data_query_element",function(){var a=$(this).attr("data-id"),c=url+"&page=query";a=getDataQueryParams(a);post(c,a)});$("div#dashboard").on("click touchstart",
".dashboard_data_explorer_element",function(){var a=$(this).attr("data-id"),c=url+"&page=explorer";a=getDataExplorerParams(a);post(c,a)});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(d){d=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,d,a)})});$(document).on("change",".element_option",function(a){var d=$(this).attr("id");a=d.replace(/^\d+_/,"");d=d.replace("_"+a,"");var f;"hide_mobile"==a?(f=$(this).prop("checked"))?$("div#element_"+
d+" div.item-content").addClass("hide_mobile"):$("div#element_"+d+" div.item-content").removeClass("hide_mobile"):f=$(this).val();changeElementAttribute(c,d,a,f)});$(document).on("click",".marker_colour",function(a){var d=$(this).attr("id");a=d.replace(/^\d+_/,"");d=d.replace("_"+a,"");a=a.replace(/^marker_/,"");changeElementAttribute(c,d,"marker_colour",a)});$(document).on("click change touchstart mouseup keyup",".marker_size",function(a){a=$(this).attr("id");a.replace("_"+d,"");var d=$(this).slider("option",
"value");a=a.replace("_marker_size","");changeElementAttribute(c,a,"marker_size",d)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,c=a.replace("_trigger","_panel");"none"==$("#"+c).css("display")?($("#"+c).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+c).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",
function(){$.get(url+"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(c);loadNewElements()});$("a#expand_trigger").click(function(a){a=250;2E3<$("#main_container").width()&&(a=500);setTimeout(function(){setGridMargins(c);c.refreshItems().layout()},a)});setGridMargins(c);window.dispatchEvent(new Event("resize"))});
...
function showOrHideElements(){var a=$(window).width()<MOBILE_WIDTH;$("div.hide_mobile").css("display",a?"none":"block");$("div.hide_border").css("border",a?"0":"1px solid var(--border-card)");$.each(elements,function(a,d){"setup"==d.display&&$("div#element_"+d.id+" div.item-content").css("display","block")})}
...
c,"display","setup"))}
function reloadElement(a){var c=url+"&page=dashboard&ajax=1&type="+dashboard_type+"&v="+version+"&open_new="+($("#open_new").prop("checked")?1:0);"undefined"!==typeof projectId&&(c+="&project_id="+projectId);c+="&element="+a;null!=qryFile&&qryFile.length&&(c+="&qry_file="+qryFile);null!=listFile&&listFile.length&&null!=listAttribute&&listAttribute.length&&(c+="&list_file="+listFile+"&list_attribute="+listAttribute);$.get(c,function(c){try{$("div#element_"+a+"> .item-content > .ajax_content").html(JSON.parse(c).html),elements[a]=
JSON.parse(c).element,applyFormatting()}catch(e){console.log(e.message)}})}function reloadAllElements(){$.each(Object.keys(elements),function(a,c){reloadElement(c)})}function reloadElementsWithURL(){$.each(Object.keys(elements),function(a,c){"specific values"!=elements[c].visualisation_type&&"top"==elements[c].breakdown_display&&reloadElement(c)})}
...
"inline")}})},c)}function resetDefaults(){$("#modify_dashboard_panel").toggle("slide",{direction:"right"},"fast",function(){$("#modal_overlay").removeClass("open")});var a=url+"&resetDefaults=1&type="+dashboard_type;"undefined"!==typeof projectId&&(a+="&project_id="+projectId);$.get(a,function(){$("input#search").length?$("input#search").trigger("click"):location.reload()})}
