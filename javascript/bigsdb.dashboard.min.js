...
$jscomp.iteratorFromArray=function(a,b){$jscomp.initSymbolIterator();a instanceof String&&(a+="");var c=0,d={next:function(){if(c<a.length){var e=c++;return{value:b(e,a[e]),done:!1}}d.next=function(){return{done:!0,value:void 0}};return d.next()}};d[Symbol.iterator]=function(){return d};return d};$jscomp.polyfill("Array.prototype.keys",function(a){return a?a:function(){return $jscomp.iteratorFromArray(this,function(a){return a})}},"es6","es3");var currentRequest=null,MOBILE_WIDTH=480;
$(function(){showOrHideElements();$("select#add_field,label[for='add_field']").css("display","inline");var a=$("#layout").val(),b=$("#fill_gaps").prop("checked"),c=$("#open_new").prop("checked");try{var d=(new Muuri("#dashboard",{dragEnabled:!0,layout:{alignRight:a.includes("right"),alignBottom:a.includes("bottom"),fillGaps:b},dragStartPredicate:function(a,b){return enable_drag}})).on("move",function(){setGridMargins(d);saveLayout(d)});order&&loadLayout(d,order)}catch(e){console.log(e.message)}$("#panel_trigger,#close_trigger").click(function(){$("#modify_panel").toggle("slide",
{direction:"right"},"fast");$("#panel_trigger").show();return!1});$("#panel_trigger").show();$(document).mouseup(function(a){var b=$("#modify_panel");b.is(a.target)||0!==b.has(a.target).length||b.hide()});$("#layout").change(function(){a=$("#layout").val();try{d._settings.layout.alignRight=a.includes("right"),d._settings.layout.alignBottom=a.includes("bottom"),d.layout()}catch(e){}$.ajax(url+"&page=dashboard&updateDashboard=1&attribute=layout&value="+a)});$("#fill_gaps").change(function(){b=$("#fill_gaps").prop("checked");
try{d._settings.layout.fillGaps=b,d.layout()}catch(e){}$.ajax(url+"&page=dashboard&updateDashboard=1&attribute=fill_gaps&value="+(b?1:0))});$("#enable_drag").change(function(){enable_drag=$("#enable_drag").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=enable_drag&value="+(enable_drag?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=edit_elements&value="+(a?1:0));$("span.dashboard_edit_element").css("display",
a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#open_new").change(function(){c=$("#open_new").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(c?1:0)}).done(function(){reloadElementsWithURL(d)})});$("#include_old_versions").change(function(){var a=
$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(){reloadAllElements(d)})});$("#add_element").click(function(){var a=getNextid();addElement(d,a);setGridMargins(d)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");
removeElement(d,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_explore_element",function(){var a=$(this).attr("data-id");if(elements[a].url){var b=elements[a].url,c={};elements[a].post_data&&(c=elements[a].post_data);c.sent=1;b.includes("&page=query")&&$("#include_old_versions").prop("checked")&&(c.include_old="on");post(elements[a].url,c)}});applyFormatting();["width",
"height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(d,b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");var c;"hide_mobile"==a?(c=$(this).prop("checked"))?$("div#element_"+b+" div.item-content").addClass("hide_mobile"):$("div#element_"+b+" div.item-content").removeClass("hide_mobile"):c=$(this).val();changeElementAttribute(d,
b,a,c)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger","_panel");"none"==$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+b).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",
function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(d);loadNewElements()});setGridMargins(d)});function setGridMargins(a){var b=155*Math.floor($("div#dashboard_panel").width()/155);$("div#dashboard").css("width",b);a.on("layoutEnd",function(){a.off("layoutEnd");var c=0;a.getItems().forEach(function(a){a=a.getPosition().left+a.getWidth();c=Math.max(c,a)});300>c&&(c=300);c+=10;c>b?$("div#dashboard").css("width",b):$("div#dashboard").css("width",c)})}
...
function post(a,b){var c=$("<form></form>");c.attr("method","post");c.attr("action",a);c.attr("target",$("#open_new").prop("checked")?"_blank":null);$.each(b,function(a,b){var d=$("<input></input>");d.attr("type","hidden");d.attr("name",a);d.attr("value",b);c.append(d)});$(document.body).append(c);c.submit()}function clean_value(a){if(null!=a)return Array.isArray(a)?(a=a.map(function(a){return a.trim()}),a=a.filter(function(a){return null!=a&&""!=a})):a=a.trim(),a}
...
function addElement(a,b){0===Object.keys(elements).length&&$("div#empty").html("");var c=url+"&page=dashboard&new="+b,d=$("#add_field").val();d&&(c+="&field="+d);$.get(c,function(c){try{var d=document.createRange().createContextualFragment(JSON.parse(c).html);b in elements||(a.add([d.firstChild]),elements[b]=JSON.parse(c).element,saveElements(a));applyFormatting();$("div#element_"+b+" div.item-content").css("visibility","visible")}catch(g){console.log(g.message)}})}
...
"record_count"==elements[a].display?($("fieldset#change_duration_control,fieldset#design_control").css("display","inline"),$("li#text_colour_control,li#background_colour_control,li#watermark_control").css("display","block")):"specific values"===b?($("li#specific_value_display_selector,li#value_selector").css("display","block"),"gauge"===c?($("fieldset#design_control").css("display","inline"),$("li.gauge_colour").css("display","block")):"number"===c&&($("fieldset#change_duration_control,fieldset#design_control").css("display",
"inline"),$("li#watermark_control,li#text_colour_control,li#background_colour_control").css("display","block"))):"breakdown"===b&&($("li#breakdown_display_selector").css("display","block"),"bar"===d?($("fieldset#design_control,li#bar_colour_type").css("display","inline"),"continuous"===$("input[name='"+a+"_bar_colour_type']:checked").val()&&$("li#chart_colour").css("display","block")):"cumulative"===d?($("fieldset#design_control").css("display","inline"),$("li#chart_colour").css("display","block")):
"map"===d?($("fieldset#design_control").css("display","inline"),$("li#palette_control").css("display","block"),show_palette(a)):"top"===d&&$("li#top_value_selector").css("display","block"))}
...
function reloadAllElements(a){$.each(Object.keys(elements),function(b,c){reloadElement(a,c)})}function reloadElementsWithURL(a){$.each(Object.keys(elements),function(b,c){"specific values"!=elements[c].visualisation_type&&"top"==elements[c].breakdown_display&&reloadElement(a,c)})}function loadNewElements(a){$.each(Object.keys(elements),function(b,c){loadedElements[c]||$("div#dashboard").width()<MOBILE_WIDTH&&elements[c].hide_mobile||(reloadElement(a,c),loadedElements[c]=1)})}
...
function saveElements(a){$.post(url,{db:instance,page:"dashboard",updateDashboard:1,attribute:"elements",value:JSON.stringify(elements)});saveLayout(a)}function saveAndReloadElement(a,b){currentRequest=$.ajax({url:url,type:"POST",data:{db:instance,page:"dashboard",updateDashboard:1,attribute:"elements",value:JSON.stringify(elements)},beforeSend:function(){null!=currentRequest&&currentRequest.abort()},success:function(){reloadElement(a,b);setGridMargins(a)}})}
function serializeLayout(a){a=a.getItems().map(function(a){return a.getElement().getAttribute("data-id")});return JSON.stringify(a)}function loadLayout(a,b){b=JSON.parse(b);for(var c=a.getItems(),d=c.map(function(a){return a.getElement().getAttribute("data-id")}),e=[],f,g=0;g<b.length;g++)f=b[g],f=d.indexOf(f),-1<f&&e.push(c[f]);a.sort(e,{layout:"instant"})}function saveLayout(a){a=serializeLayout(a);$.post(url,{db:instance,page:"dashboard",updateDashboard:1,attribute:"order",value:a})}
