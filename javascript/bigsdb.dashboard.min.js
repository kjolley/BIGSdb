...
b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");changeElementAttribute(d,b,a,$(this).val())});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updatePrefs=1&attribute=default&value=0",function(){window.location=url})})});
...
function editElement(a,b,c){$("span#control_"+b).hide();$("span#wait_"+b).show();$.get(url+"&page=dashboard&control="+b,function(a){$(a).appendTo("body").modal();$("#edit_elements").prop("checked")&&$("span#control_"+b).show();$("span#wait_"+b).hide();$("div.modal").on("change","#"+b+"_visualisation_type",function(){$("li#"+b+"_value_selector").css("display","breakdown"===$("#"+b+"_visualisation_type:checked").val()?"none":"inline")});$("div.modal").on($.modal.AFTER_CLOSE,function(a,b){$("div.modal").remove()})})}
