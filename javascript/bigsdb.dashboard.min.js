...
change:function(a,c){a=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;a+="&attribute=record_age&value="+c.value;"undefined"!==typeof projectId&&(a+="&project_id="+projectId);$.ajax({url:a}).done(function(a){$("#loaded_dashboard").val(JSON.parse(a).dashboard_name);$("#loaded_dashboard").prop("disabled",!1);$("#delete_dashboard").css("display","inline");$("#filter_age").html(recordAgeLabels[c.value]);reloadAllElements()})}});$("#dashboard_panel_trigger,#close_dashboard_trigger").click(function(){$("#modify_dashboard_panel").toggle("slide",
...
$("#include_old_versions").prop("checked"),c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=include_old_versions&value="+(a?1:0))}).done(function(c){updateDashboardName(JSON.parse(c).dashboard_name);$("#filter_versions").html(a?"all":"current");reloadAllElements()})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val(),c=url+"&page=dashboard&type="+dashboard_type;"undefined"!==
typeof projectId&&(c+="&project_id="+projectId);c+="&updateDashboardName="+encodeURIComponent(a);a.length&&$.ajax(c)});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();a=url+"&page=dashboard&setActiveDashboard="+a+"&type="+dashboard_type;"undefined"!==typeof projectId&&(a+="&project_id="+projectId);$.ajax({url:a}).done(function(){$("input#search").length?$("input#search").trigger("click"):location.reload()})});$("#add_element").click(function(){var a=getNextid();addElement(d,
a);setGridMargins(d)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(d,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_data_query_element",function(){var a=$(this).attr("data-id"),
c=url+"&page=query";a=getDataQueryParams(a);post(c,a)});$("div#dashboard").on("click touchstart",".dashboard_data_explorer_element",function(){var a=$(this).attr("data-id"),c=url+"&page=explorer";a=getDataExplorerParams(a);post(c,a)});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(c){c=$(this).attr("id").replace("_"+a,"");changeElementDimension(d,c,a)})});$(document).on("change",".element_option",function(a){var c=$(this).attr("id");a=c.replace(/^\d+_/,
"");c=c.replace("_"+a,"");var e;"hide_mobile"==a?(e=$(this).prop("checked"))?$("div#element_"+c+" div.item-content").addClass("hide_mobile"):$("div#element_"+c+" div.item-content").removeClass("hide_mobile"):e=$(this).val();changeElementAttribute(d,c,a,e)});$(document).on("click",".marker_colour",function(a){var c=$(this).attr("id");a=c.replace(/^\d+_/,"");c=c.replace("_"+a,"");a=a.replace(/^marker_/,"");changeElementAttribute(d,c,"marker_colour",a)});$(document).on("click change touchstart mouseup keyup",
...
c,"display","setup"))}
function reloadElement(a){var c=url+"&page=dashboard&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);c+="&element="+a;null!=qryFile&&qryFile.length&&(c+="&qry_file="+qryFile);null!=listFile&&listFile.length&&null!=listAttribute&&listAttribute.length&&(c+="&list_file="+listFile+"&list_attribute="+listAttribute);$.get(c,function(c){try{$("div#element_"+a+"> .item-content > .ajax_content").html(JSON.parse(c).html),elements[a]=JSON.parse(c).element,applyFormatting()}catch(e){console.log(e.message)}})}
function reloadAllElements(){$.each(Object.keys(elements),function(a,c){reloadElement(c)})}function reloadElementsWithURL(){$.each(Object.keys(elements),function(a,c){"specific values"!=elements[c].visualisation_type&&"top"==elements[c].breakdown_display&&reloadElement(c)})}function loadNewElements(){$.each(Object.keys(elements),function(a,c){loadedElements[c]||$(window).width()<MOBILE_WIDTH&&elements[c].hide_mobile||(reloadElement(c),loadedElements[c]=1)})}
function removeElement(a,c){var d=a.getItem($("div#element_"+c)[0]);a.remove([d],{removeElements:!0});delete elements[c];saveElements(a);0==Object.keys(elements).length&&$("div#empty").html(empty)}
