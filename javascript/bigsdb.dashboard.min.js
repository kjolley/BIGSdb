...
function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_explore_element",function(){var a=$(this).attr("data-id");if(elements[a].url){var b=elements[a].url,c={};elements[a].post_data&&(c=elements[a].post_data);
c.sent=1;b.includes("&page=query")&&($("#include_old_versions").prop("checked")&&(c.include_old="on"),b=$("#record_age_slider").slider("value"),0<b&&(c.prov_field1="f_date_entered",c.prov_operator1=">=",c.prov_value1=datestamps[b]));post(elements[a].url,c)}});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,b,a)})});$(document).on("change",".element_option",function(a){var b=
$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");var d;"hide_mobile"==a?(d=$(this).prop("checked"))?$("div#element_"+b+" div.item-content").addClass("hide_mobile"):$("div#element_"+b+" div.item-content").removeClass("hide_mobile"):d=$(this).val();changeElementAttribute(c,b,a,d)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger",
"_panel");"none"==$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+b).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(c);loadNewElements()});setGridMargins(c)});
...
function addElement(a,b){0===Object.keys(elements).length&&$("div#empty").html("");var c=url+"&page=dashboard&new="+b,d=$("#add_field").val();d&&(c+="&field="+d);lastAjaxUpdate=(new Date).getTime();$.get(c,function(c){try{var d=document.createRange().createContextualFragment(JSON.parse(c).html);b in elements||(a.add([d.firstChild]),elements[b]=JSON.parse(c).element,saveElements(a));applyFormatting();$("div#element_"+b+" div.item-content").css("visibility","visible");updateDashboardName(JSON.parse(c).dashboard_name);
