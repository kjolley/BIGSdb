...
{direction:"right"},"fast",function(){$("#modify_dashboard_panel").is(":visible")?$("#modal_overlay").addClass("open"):$("#modal_overlay").removeClass("open")});$("#dashboard_panel_trigger").show();return!1});$("#dashboard_panel_trigger").show();$(document).mouseup(function(a){var c=$("#modify_dashboard_panel");!c.is(a.target)&&0===c.has(a.target).length&&c.is(":visible")&&(c.hide(),$("#modal_overlay").removeClass("open"))});$("#fill_gaps").change(function(){a=$("#fill_gaps").prop("checked");try{d._settings.layout.fillGaps=
a,d.layout()}catch(f){}var c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);c+="&attribute=fill_gaps&value="+(a?1:0);$.ajax({url:c}).done(function(a){updateDashboardName(JSON.parse(a).dashboard_name)})});$("#enable_drag").change(function(){enable_drag=$("#enable_drag").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=enable_drag&value="+(enable_drag?1:0)})});$("#edit_elements").change(function(){var a=
$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=edit_elements&value="+(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#open_new").change(function(){c=$("#open_new").prop("checked");
$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(c?1:0)}).done(function(){reloadElementsWithURL()})});$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked"),c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=include_old_versions&value="+(a?1:0))}).done(function(c){updateDashboardName(JSON.parse(c).dashboard_name);$("#filter_versions").html(a?
"all":"current");reloadAllElements();bumpVersion()})});$("#dashboard_palette").change(function(){var a=$("#dashboard_palette").val(),c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=palette&value="+a)}).done(function(a){updateDashboardName(JSON.parse(a).dashboard_name);bumpVersion();reloadAllElements()})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val(),c=url+"&page=dashboard&type="+
dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);c+="&updateDashboardName="+encodeURIComponent(a);a.length&&$.ajax(c)});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();a=url+"&page=dashboard&setActiveDashboard="+a+"&type="+dashboard_type;"undefined"!==typeof projectId&&(a+="&project_id="+projectId);$.ajax({url:a}).done(function(){$("input#search").length?$("input#search").trigger("click"):location.reload()})});$("#add_element").click(function(){var a=
getNextid();addElement(d,a);setGridMargins(d)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(d,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_data_query_element",function(){var a=
$(this).attr("data-id"),c=url+"&page=query";a=getDataQueryParams(a);post(c,a)});$("div#dashboard").on("click touchstart",".dashboard_data_explorer_element",function(){var a=$(this).attr("data-id"),c=url+"&page=explorer";a=getDataExplorerParams(a);post(c,a)});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(c){c=$(this).attr("id").replace("_"+a,"");changeElementDimension(d,c,a)})});$(document).on("change",".element_option",function(a){var c=
$(this).attr("id");a=c.replace(/^\d+_/,"");c=c.replace("_"+a,"");var e;"hide_mobile"==a?(e=$(this).prop("checked"))?$("div#element_"+c+" div.item-content").addClass("hide_mobile"):$("div#element_"+c+" div.item-content").removeClass("hide_mobile"):e=$(this).val();changeElementAttribute(d,c,a,e)});$(document).on("click",".marker_colour",function(a){var c=$(this).attr("id");a=c.replace(/^\d+_/,"");c=c.replace("_"+a,"");a=a.replace(/^marker_/,"");changeElementAttribute(d,c,"marker_colour",a)});$(document).on("click change touchstart mouseup keyup",
".marker_size",function(a){a=$(this).attr("id");a.replace("_"+c,"");var c=$(this).slider("option","value");a=a.replace("_marker_size","");changeElementAttribute(d,a,"marker_size",c)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,c=a.replace("_trigger","_panel");"none"==$("#"+c).css("display")?($("#"+c).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):
($("#"+c).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(d);loadNewElements()});$("a#expand_trigger").click(function(a){a=250;2E3<$("#main_container").width()&&(a=500);setTimeout(function(){setGridMargins(d);d.refreshItems().layout()},a)});setGridMargins(d);window.dispatchEvent(new Event("resize"))});
