...
function post(a,b){var c=$("<form></form>");c.attr("method","post");c.attr("action",a);$.each(b,function(a,b){var d=$("<input></input>");d.attr("type","hidden");d.attr("name",a);d.attr("value",b);c.append(d)});$(document.body).append(c);c.submit()}function clean_value(a){Array.isArray(a)?(a=a.map(function(a){return a.trim()}),a=a.filter(function(a){return null!=a&&""!=a})):a=a.trim();return a}
...
function editElement(a,b,c){$("span#control_"+b).hide();$("span#wait_"+b).show();$.get(url+"&page=dashboard&control="+b,function(c){$(c).appendTo("body").modal();$("#edit_elements").prop("checked")&&$("span#control_"+b).show();$("span#wait_"+b).hide();show_or_hide_control_elements(a,b);$("div.modal").on("change","#"+b+"_visualisation_type",function(){show_or_hide_control_elements(a,b);check_and_show_visualisation(a,b)});$("div.modal").on("change","#"+b+"_breakdown_display,#"+b+"_specific_value_display,#"+
b+"_specific_values",function(){show_or_hide_control_elements(a,b);check_and_show_visualisation(a,b)});$("div.modal").on($.modal.AFTER_CLOSE,function(a,b){$("div.modal").remove()})})}
