...
$(function(){showOrHideElements();$("select#add_field,label[for='add_field']").css("display","inline");var a=$("#layout").val(),b=$("#fill_gaps").prop("checked");try{var c=(new Muuri("#dashboard",{dragEnabled:!0,layout:{alignRight:a.includes("right"),alignBottom:a.includes("bottom"),fillGaps:b},dragStartPredicate:function(a,b){return enable_drag}})).on("move",function(){setGridMargins(c);saveLayout(c)});order&&loadLayout(c,order)}catch(d){console.log(d.message)}$("#panel_trigger,#close_trigger").click(function(){$("#modify_panel").toggle("slide",
{direction:"right"},"fast");$("#panel_trigger").show();return!1});$("#panel_trigger").show();$(document).mouseup(function(a){var b=$("#modify_panel");b.is(a.target)||0!==b.has(a.target).length||b.hide()});$("#layout").change(function(){a=$("#layout").val();try{c._settings.layout.alignRight=a.includes("right"),c._settings.layout.alignBottom=a.includes("bottom"),c.layout()}catch(d){}$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=layout&value="+a)});$("#fill_gaps").change(function(){b=$("#fill_gaps").prop("checked");
try{c._settings.layout.fillGaps=b,c.layout()}catch(d){}$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=fill_gaps&value="+(b?1:0))});$("#enable_drag").change(function(){enable_drag=$("#enable_drag").prop("checked");$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=enable_drag&value="+(enable_drag?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=edit_elements&value="+(a?1:0));$("span.dashboard_edit_element").css("display",
a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updatePrefs=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(){reloadAllElements(c)})});$("#add_element").click(function(){var a=
getNextid();addElement(c,a);setGridMargins(c)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_explore_element",function(){var a=
$(this).attr("data-id");if(elements[a].url){var b=elements[a].url,c={};elements[a].post_data&&(c=elements[a].post_data);c.sent=1;b.includes("&page=query")&&$("#include_old_versions").prop("checked")&&(c.include_old="on");post(elements[a].url,c)}});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");
a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");var d;"hide_mobile"==a?(d=$(this).prop("checked"))?$("div#element_"+b+" div.item-content").addClass("hide_mobile"):$("div#element_"+b+" div.item-content").removeClass("hide_mobile"):d=$(this).val();changeElementAttribute(c,b,a,d)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger","_panel");"none"==
$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+b).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updatePrefs=1&attribute=default&value=0",function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(c);loadNewElements()});setGridMargins(c)});
function setGridMargins(a){var b=155*Math.floor($("div#dashboard_panel").width()/155);$("div#dashboard").css("width",b);a.on("layoutEnd",function(){a.off("layoutEnd");var c=0;a.getItems().forEach(function(a){a=a.getPosition().left+a.getWidth();c=Math.max(c,a)});300>c&&(c=300);c>b?$("div#dashboard").css("width",b):$("div#dashboard").css("width",c)})}
...
function post(a,b){var c=$("<form></form>");c.attr("method","post");c.attr("action",a);$.each(b,function(a,b){var d=$("<input></input>");d.attr("type","hidden");d.attr("name",a);d.attr("value",b);c.append(d)});$(document.body).append(c);c.submit()}function clean_value(a){if(null!=a)return Array.isArray(a)?(a=a.map(function(a){return a.trim()}),a=a.filter(function(a){return null!=a&&""!=a})):a=a.trim(),a}
...
function reloadAllElements(a){$.each(Object.keys(elements),function(b,c){reloadElement(a,c)})}function loadNewElements(a){$.each(Object.keys(elements),function(b,c){loadedElements[c]||$("div#dashboard").width()<MOBILE_WIDTH&&elements[c].hide_mobile||(reloadElement(a,c),loadedElements[c]=1)})}function removeElement(a,b){var c=a.getItem($("div#element_"+b)[0]);a.remove([c],{removeElements:!0});delete elements[b];saveElements(a);0==Object.keys(elements).length&&$("div#empty").html(empty)}
