...
a.includes("right");grid._settings.layout.alignBottom=a.includes("bottom");grid.layout();$.ajax(ajax_url+"&attribute=layout&value="+a)});$("#fill_gaps").change(function(){b=$("#fill_gaps").prop("checked");grid._settings.layout.fillGaps=b;grid.layout();$.ajax(ajax_url+"&attribute=fill_gaps&value="+(b?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(ajax_url+"&attribute=edit_elements&value="+(a?1:0));$("span.dashboard_control").css("display",a?"inline":
"none")});$(".dashboard_control").click(function(){var a=$(this).attr("data-id");$("span#control_"+a).hide();$("span#wait_"+a).show();event.preventDefault();this.blur();$.get(modal_control_url+"&control="+a,function(c){$(c).appendTo("body").modal();$("span#control_"+a).show();$("span#wait_"+a).hide()})});["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(b,a)})});$("a#dashboard_toggle").on("click",
...
function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(reset_url,function(){$("#layout").val("left-top");$("#fill_gaps").prop("checked",!0);$("#edit_elements").prop("checked",!1);location.reload()})};
