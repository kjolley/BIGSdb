...
change:function(a,b){$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=record_age&value="+b.value}).done(function(){$("#filter_age").html(recordAgeLabels[b.value]);reloadAllElements()})}});$("#panel_trigger,#close_trigger").click(function(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$("#panel_trigger").show();return!1});$("#panel_trigger").show();$(document).mouseup(function(a){var b=$("#modify_panel");b.is(a.target)||0!==b.has(a.target).length||b.hide()});$("#fill_gaps").change(function(){a=
$("#fill_gaps").prop("checked");try{c._settings.layout.fillGaps=a,c.layout()}catch(d){}$.ajax(url+"&page=dashboard&updateDashboard=1&attribute=fill_gaps&value="+(a?1:0))});$("#enable_drag").change(function(){enable_drag=$("#enable_drag").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=enable_drag&value="+(enable_drag?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=edit_elements&value="+
(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#open_new").change(function(){b=$("#open_new").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(b?1:0)}).done(function(){reloadElementsWithURL()})});
$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(){$("#filter_versions").html(a?"all":"current");reloadAllElements()})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val();a.length&&$.ajax(url+"&page=dashboard&updateDashboardName="+encodeURIComponent(a))});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();
$.ajax({url:url+"&page=dashboard&setActiveDashboard="+a}).done(function(){location.reload()})});$("#add_element").click(function(){var a=getNextid();addElement(c,a);setGridMargins(c)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click touchstart",".setup_element",
function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_explore_element",function(){var a=$(this).attr("data-id");if(elements[a].url){var b=elements[a].url,c={};elements[a].post_data&&(c=elements[a].post_data);c.sent=1;b.includes("&page=query")&&($("#include_old_versions").prop("checked")&&(c.include_old="on"),b=$("#record_age_slider").slider("value"),0<b&&(c.prov_field1="f_date_entered",c.prov_operator1=">=",c.prov_value1=datestamps[b]));post(elements[a].url,
c)}});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");var d;"hide_mobile"==a?(d=$(this).prop("checked"))?$("div#element_"+b+" div.item-content").addClass("hide_mobile"):$("div#element_"+b+" div.item-content").removeClass("hide_mobile"):d=$(this).val();
changeElementAttribute(c,b,a,d)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger","_panel");"none"==$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+b).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",
function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(c);loadNewElements()});setGridMargins(c)});function setGridMargins(a){var b=155*Math.floor($("div#dashboard_panel").width()/155);$("div#dashboard").css("width",b);a.on("layoutEnd",function(){a.off("layoutEnd");var c=0;a.getItems().forEach(function(a){a=a.getPosition().left+a.getWidth();c=Math.max(c,a)});300>c&&(c=300);c+=10;c>b?$("div#dashboard").css("width",b):$("div#dashboard").css("width",c)})}
...
function addElement(a,b){0===Object.keys(elements).length&&$("div#empty").html("");var c=url+"&page=dashboard&new="+b,d=$("#add_field").val();d&&(c+="&field="+d);lastAjaxUpdate=(new Date).getTime();$.get(c,function(c){try{var d=document.createRange().createContextualFragment(JSON.parse(c).html);b in elements||(a.add([d.firstChild]),elements[b]=JSON.parse(c).element,saveElements(a));applyFormatting();$("div#element_"+b+" div.item-content").css("visibility","visible");var e=JSON.parse(c).dashboard_name;
$("#loaded_dashboard").val(e);$("#loaded_dashboard").prop("disabled",!1);$("#delete_dashboard").css("display","inline")}catch(h){console.log(h.message)}})}
...
"record_count"==elements[a].display?($("fieldset#change_duration_control,fieldset#design_control").css("display","inline"),$("li#text_colour_control,li#background_colour_control,li#watermark_control").css("display","block")):"seqbin_size"==elements[a].display?($("fieldset#design_control").css("display","inline"),$("li#chart_colour").css("display","block")):"specific values"===b?($("li#specific_value_display_selector,li#value_selector").css("display","block"),"gauge"===c?($("fieldset#design_control").css("display",
"inline"),$("li.gauge_colour").css("display","block")):"number"===c&&($("fieldset#change_duration_control,fieldset#design_control").css("display","inline"),$("li#watermark_control,li#text_colour_control,li#background_colour_control").css("display","block"))):"breakdown"===b&&($("li#breakdown_display_selector").css("display","block"),"bar"===d?($("fieldset#design_control,li#bar_colour_type").css("display","inline"),"continuous"===$("input[name='"+a+"_bar_colour_type']:checked").val()&&$("li#chart_colour").css("display",
"block")):"cumulative"===d?($("fieldset#design_control").css("display","inline"),$("li#chart_colour").css("display","block")):"map"===d?($("fieldset#design_control").css("display","inline"),$("li#palette_control").css("display","block"),show_palette(a)):"top"===d&&$("li#top_value_selector").css("display","block"))}
...
function saveElements(a){$.post(url,{db:instance,page:"dashboard",updateDashboard:1,attribute:"elements",value:JSON.stringify(elements)});saveLayout(a)}function saveAndReloadElement(a,b){currentRequest=$.ajax({url:url,type:"POST",data:{db:instance,page:"dashboard",updateDashboard:1,attribute:"elements",value:JSON.stringify(elements)},beforeSend:function(){null!=currentRequest&&currentRequest.abort()},success:function(){reloadElement(b);null!=a&&setGridMargins(a)}})}
...
function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(url+"&resetDefaults=1",function(){location.reload()})}function createNew(){$.ajax({url:url+"&newDashboard=1",type:"GET",success:function(){location.reload()}})}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
