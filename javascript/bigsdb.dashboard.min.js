...
$("#include_old_versions").prop("checked"),c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=include_old_versions&value="+(a?1:0))}).done(function(c){updateDashboardName(JSON.parse(c).dashboard_name);$("#filter_versions").html(a?"all":"current");reloadAllElements()})});$("#dashboard_palette").change(function(){var a=$("#dashboard_palette").val(),c=url+"&page=dashboard&updateDashboard=1&type="+dashboard_type;
"undefined"!==typeof projectId&&(c+="&project_id="+projectId);$.ajax({url:c+("&attribute=palette&value="+a)}).done(function(){reloadAllElements()})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val(),c=url+"&page=dashboard&type="+dashboard_type;"undefined"!==typeof projectId&&(c+="&project_id="+projectId);c+="&updateDashboardName="+encodeURIComponent(a);a.length&&$.ajax(c)});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();a=url+"&page=dashboard&setActiveDashboard="+
a+"&type="+dashboard_type;"undefined"!==typeof projectId&&(a+="&project_id="+projectId);$.ajax({url:a}).done(function(){$("input#search").length?$("input#search").trigger("click"):location.reload()})});$("#add_element").click(function(){var a=getNextid();addElement(d,a);setGridMargins(d)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=
$(this).attr("data-id");removeElement(d,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(d,a)});$("div#dashboard").on("click touchstart",".dashboard_data_query_element",function(){var a=$(this).attr("data-id"),c=url+"&page=query";a=getDataQueryParams(a);post(c,a)});$("div#dashboard").on("click touchstart",".dashboard_data_explorer_element",function(){var a=$(this).attr("data-id"),c=url+"&page=explorer";a=getDataExplorerParams(a);post(c,
a)});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(c){c=$(this).attr("id").replace("_"+a,"");changeElementDimension(d,c,a)})});$(document).on("change",".element_option",function(a){var c=$(this).attr("id");a=c.replace(/^\d+_/,"");c=c.replace("_"+a,"");var e;"hide_mobile"==a?(e=$(this).prop("checked"))?$("div#element_"+c+" div.item-content").addClass("hide_mobile"):$("div#element_"+c+" div.item-content").removeClass("hide_mobile"):e=$(this).val();
changeElementAttribute(d,c,a,e)});$(document).on("click",".marker_colour",function(a){var c=$(this).attr("id");a=c.replace(/^\d+_/,"");c=c.replace("_"+a,"");a=a.replace(/^marker_/,"");changeElementAttribute(d,c,"marker_colour",a)});$(document).on("click change touchstart mouseup keyup",".marker_size",function(a){a=$(this).attr("id");a.replace("_"+c,"");var c=$(this).slider("option","value");a=a.replace("_marker_size","");changeElementAttribute(d,a,"marker_size",c)});$(document).on("change",".palette_selector",
function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,c=a.replace("_trigger","_panel");"none"==$("#"+c).css("display")?($("#"+c).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+c).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",function(){window.location=
url})});$(window).resize(function(){showOrHideElements();setGridMargins(d);loadNewElements()});setGridMargins(d);window.dispatchEvent(new Event("resize"))});
