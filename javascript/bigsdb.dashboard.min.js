...
$("#open_new").change(function(){b=$("#open_new").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(b?1:0)}).done(function(){reloadElementsWithURL(c)})});$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(){reloadAllElements(c)})});$("#add_element").click(function(){var a=getNextid();addElement(c,
...
function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(url+"&resetDefaults=1",function(){$("#fill_gaps").prop("checked",!0);$("#enable_drag").prop("checked",!1);$("#open_new").prop("checked",!0);$("#edit_elements").prop("checked",!0);$("#remove_elements").prop("checked",!1);$("#include_old_versions").prop("checked",!1);location.reload()})}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
