...
return b}function getDataExplorerParams(a){a={page:"explorer"};a.db=instance;return a}function updateDashboardName(a){$("#loaded_dashboard").val(a);$("#loaded_dashboard").prop("disabled",!1);$("#delete_dashboard").css("display","inline")}
...
function post(a,b){var c=detectAndroidFirefox()?"GET":"POST",d=$("<form></form>");$.each(b,function(a,b){var c=$("<input></input>");c.attr("type","hidden");c.attr("name",a);c.attr("value",b);d.append(c)});d.attr("method",c);d.attr("action",a);d.attr("target",$("#open_new").prop("checked")?"_blank":null);$(document.body).append(d);d.submit()}function detectAndroidFirefox(){var a=navigator.userAgent.toLowerCase();return 0<=a.indexOf("firefox")?0<=a.indexOf("android")?!0:!1:!1}
