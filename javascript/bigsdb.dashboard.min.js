...
a,function(c){$(c).appendTo("body").modal();$("span#control_"+a).show();$("span#wait_"+a).hide()})});["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(b,a)})});$("a#dashboard_toggle").on("click",function(){$.get(ajax_url+"&attribute=default&value=0",function(){window.location=url+"?db="+instance})})});
