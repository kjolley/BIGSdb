...
function getDataQueryParams(a){var b={};elements[a].post_data&&(b=elements[a].post_data);b.page="query";b.db=instance;b.sent=1;$("#include_old_versions").prop("checked")&&(b.include_old="on");a=$("#record_age_slider").slider("value");0<a&&(b.prov_field1="f_date_entered",b.prov_operator1=">=",b.prov_value1=datestamps[a]);return b}function getDataExplorerParams(a){a={page:"explorer"};a.db=instance;return a}
...
function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(url+"&resetDefaults=1",function(){location.reload()})}function resetSeqbinRange(a){delete elements[a].min;delete elements[a].max;saveAndReloadElement(null,a);$.ajax({url:url+"&seqbin_range=1",type:"GET",success:function(a){a=JSON.parse(a).range;$("#seqbin_range_slider").slider("option","values",[a.min,a.max]);$("#seqbin_min").html(a.min);$("#seqbin_max").html(a.max)}})}
function createNew(){$.ajax({url:url+"&newDashboard=1",type:"GET",success:function(){location.reload()}})}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
