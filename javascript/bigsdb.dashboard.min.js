...
$(function(){var a=$("#layout").val(),b=$("#fill_gaps").prop("checked");try{var c=(new Muuri(".grid",{dragEnabled:!0,layout:{alignRight:a.includes("right"),alignBottom:a.includes("bottom"),fillGaps:b}})).on("move",function(){saveLayout(c)});order&&loadLayout(c,order)}catch(d){console.log(d.message)}$("#panel_trigger,#close_trigger").click(function(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$("#panel_trigger").show();return!1});$("#panel_trigger").show();$(document).mouseup(function(a){var b=
$("#modify_panel");b.is(a.target)||0!==b.has(a.target).length||b.hide()});$("#layout").change(function(){a=$("#layout").val();try{c._settings.layout.alignRight=a.includes("right"),c._settings.layout.alignBottom=a.includes("bottom"),c.layout()}catch(d){}$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=layout&value="+a)});$("#fill_gaps").change(function(){b=$("#fill_gaps").prop("checked");try{c._settings.layout.fillGaps=b,c.layout()}catch(d){}$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=fill_gaps&value="+
(b?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=edit_elements&value="+(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updatePrefs=1&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#add_element").click(function(){var a=
getNextid();addElement(c,a)});$("div#dashboard").on("click",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click",".setup_element",function(){var a=$(this).attr("data-id");setupElement(c,a)});["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");
changeElementDimension(c,b,a)})});$("a#dashboard_toggle").on("click",function(){$.get(url+"&page=dashboard&updatePrefs=1&attribute=default&value=0",function(){window.location=url})})});function getNextid(){return 0===Object.keys(elements).length?1:Math.max.apply(Math,[].concat($jscomp.arrayFromIterable(Object.keys(elements))))+1}
function addElement(a,b){0===Object.keys(elements).length&&$("div#empty").html("");var c=url+"&page=dashboard&new="+b,d=$("#add_field").val();d&&(c+="&field="+d);$.get(c,function(c){try{var d=document.createRange().createContextualFragment(JSON.parse(c).html);b in elements||(a.add([d.firstChild]),elements[b]=JSON.parse(c).element,saveElements(a))}catch(g){console.log(g.message)}})}
function editElement(a,b){$("span#control_"+b).hide();$("span#wait_"+b).show();$.get(url+"&page=dashboard&control="+b,function(a){$(a).appendTo("body").modal();$("span#control_"+b).show();$("span#wait_"+b).hide()})}function reloadElement(a,b){$.get(url+"&page=dashboard&element="+b,function(a){try{$("div#element_"+b+"> .item-content").html(JSON.parse(a).html),elements[b]=JSON.parse(a).element}catch(d){console.log(d.message)}})}
function removeElement(a,b){var c=a.getItem($("div#element_"+b)[0]);a.remove([c],{removeElements:!0});delete elements[b];saveElements(a);0==Object.keys(elements).length&&$("div#empty").html(empty)}function setupElement(a,b){$.get(url+"&page=dashboard&setup="+b,function(a){$(a).appendTo("body").modal()})}
function changeElementDimension(a,b,c){var d=$("div.item[data-id='"+b+"'] > div.item-content"),e=d.attr("class").split(/\s+/);$.each(e,function(a,b){b.includes("dashboard_element_"+c)&&d.removeClass(b)});e=$("input[name='"+b+"_"+c+"']:checked").val();d.addClass("dashboard_element_"+c+e);$("span#"+b+"_"+c).html(e);elements[b][c]=Number(e);$.post(url,{db:instance,page:"dashboard",updatePrefs:1,attribute:"elements",value:JSON.stringify(elements)});a.refreshItems().layout()}
