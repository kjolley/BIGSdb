...
$(function(){showOrHideElements();$("select#add_field,label[for='add_field']").css("display","inline");var a=$("#fill_gaps").prop("checked"),b=$("#open_new").prop("checked");try{var c=(new Muuri("#dashboard",{dragEnabled:!0,layout:{fillGaps:a},dragStartPredicate:function(a,c){return enable_drag}})).on("move",function(){setGridMargins(c);saveLayout(c)});order&&loadLayout(c,order)}catch(d){console.log(d.message)}$("#panel_trigger,#close_trigger").click(function(){$("#modify_panel").toggle("slide",{direction:"right"},
"fast");$("#panel_trigger").show();return!1});$("#panel_trigger").show();$(document).mouseup(function(a){var c=$("#modify_panel");c.is(a.target)||0!==c.has(a.target).length||c.hide()});$("#fill_gaps").change(function(){a=$("#fill_gaps").prop("checked");try{c._settings.layout.fillGaps=a,c.layout()}catch(d){}$.ajax(url+"&page=dashboard&updateDashboard=1&attribute=fill_gaps&value="+(a?1:0))});$("#enable_drag").change(function(){enable_drag=$("#enable_drag").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=enable_drag&value="+
...
$("#open_new").change(function(){b=$("#open_new").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(b?1:0)}).done(function(){reloadElementsWithURL(c)})});$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(){reloadAllElements(c)})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val();
a.length&&$.ajax(url+"&page=dashboard&updateDashboardName="+encodeURIComponent(a))});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();$.ajax({url:url+"&page=dashboard&setActiveDashboard="+a}).done(function(){location.reload()})});$("#add_element").click(function(){var a=getNextid();addElement(c,a);setGridMargins(c)});$("div#dashboard").on("click touchstart",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",
".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_explore_element",function(){var a=$(this).attr("data-id");if(elements[a].url){var c=elements[a].url,b={};elements[a].post_data&&(b=elements[a].post_data);b.sent=1;c.includes("&page=query")&&$("#include_old_versions").prop("checked")&&(b.include_old=
"on");post(elements[a].url,b)}});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");var d;"hide_mobile"==a?(d=$(this).prop("checked"))?$("div#element_"+b+" div.item-content").addClass("hide_mobile"):$("div#element_"+b+" div.item-content").removeClass("hide_mobile"):
d=$(this).val();changeElementAttribute(c,b,a,d)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger","_panel");"none"==$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):($("#"+b).slideUp(),$("#"+a).html('<span class="fas fa-plus"></span>'))});$("a#dashboard_toggle").on("click",function(){$.get(url+
"&page=dashboard&updateGeneralPrefs=1&attribute=default&value=0",function(){window.location=url})});$(window).resize(function(){showOrHideElements();setGridMargins(c);loadNewElements()});setGridMargins(c)});
...
function resetDefaults(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$.get(url+"&resetDefaults=1",function(){location.reload()})}function createNew(){$.get(url+"&newDashboard=1",function(){location.reload()})}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
