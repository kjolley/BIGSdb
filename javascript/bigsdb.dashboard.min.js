...
change:function(a,b){$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=record_age&value="+b.value}).done(function(a){$("#loaded_dashboard").val(JSON.parse(a).dashboard_name);$("#loaded_dashboard").prop("disabled",!1);$("#delete_dashboard").css("display","inline");$("#filter_age").html(recordAgeLabels[b.value]);reloadAllElements()})}});$("#dashboard_panel_trigger,#close_dashboard_trigger").click(function(){$("#modify_dashboard_panel").toggle("slide",{direction:"right"},"fast");$("#dashboard_panel_trigger").show();
return!1});$("#dashboard_panel_trigger").show();$(document).mouseup(function(a){var b=$("#modify_dashboard_panel");b.is(a.target)||0!==b.has(a.target).length||b.hide()});$("#fill_gaps").change(function(){a=$("#fill_gaps").prop("checked");try{c._settings.layout.fillGaps=a,c.layout()}catch(d){}$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=fill_gaps&value="+(a?1:0)}).done(function(a){updateDashboardName(JSON.parse(a).dashboard_name)})});$("#enable_drag").change(function(){enable_drag=
$("#enable_drag").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=enable_drag&value="+(enable_drag?1:0)})});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=edit_elements&value="+(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(url+"&page=dashboard&updateGeneralPrefs=1&attribute=remove_elements&value="+
(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#open_new").change(function(){b=$("#open_new").prop("checked");$.ajax({url:url+"&page=dashboard&updateGeneralPrefs=1&attribute=open_new&value="+(b?1:0)}).done(function(){reloadElementsWithURL()})});$("#include_old_versions").change(function(){var a=$("#include_old_versions").prop("checked");$.ajax({url:url+"&page=dashboard&updateDashboard=1&attribute=include_old_versions&value="+(a?1:0)}).done(function(b){updateDashboardName(JSON.parse(b).dashboard_name);
$("#filter_versions").html(a?"all":"current");reloadAllElements()})});$("#loaded_dashboard").change(function(){var a=$("#loaded_dashboard").val();a.length&&$.ajax(url+"&page=dashboard&updateDashboardName="+encodeURIComponent(a))});$("#switch_dashboard").change(function(){var a=$("#switch_dashboard").val();$.ajax({url:url+"&page=dashboard&setActiveDashboard="+a}).done(function(){location.reload()})});$("#add_element").click(function(){var a=getNextid();addElement(c,a);setGridMargins(c)});$("div#dashboard").on("click touchstart",
".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});$("div#dashboard").on("click touchstart",".setup_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click touchstart",".dashboard_data_query_element",function(){var a=$(this).attr("data-id"),b=url+"&page=query";a=getDataQueryParams(a);post(b,a)});
$("div#dashboard").on("click touchstart",".dashboard_data_explorer_element",function(){var a=$(this).attr("data-id"),b=url+"&page=explorer";a=getDataExplorerParams(a);post(b,a)});applyFormatting();["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,b,a)})});$(document).on("change",".element_option",function(a){var b=$(this).attr("id");a=b.replace(/^\d+_/,"");b=b.replace("_"+a,"");var d;"hide_mobile"==
a?(d=$(this).prop("checked"))?$("div#element_"+b+" div.item-content").addClass("hide_mobile"):$("div#element_"+b+" div.item-content").removeClass("hide_mobile"):d=$(this).val();changeElementAttribute(c,b,a,d)});$(document).on("change",".palette_selector",function(a){a=$(this).attr("id");a=a.replace(/_palette$/,"");show_palette(a)});$(".multi_menu_trigger").on("click",function(){var a=this.id,b=a.replace("_trigger","_panel");"none"==$("#"+b).css("display")?($("#"+b).slideDown(),$("#"+a).html('<span class="fas fa-minus"></span>')):
...
function addElement(a,b){0===Object.keys(elements).length&&$("div#empty").html("");var c=url+"&page=dashboard&new="+b,d=$("#add_field").val();d&&(c+="&field="+d);null!=qryFile&&qryFile.length&&(c+="&qry_file="+qryFile);null!=listFile&&listFile.length&&null!=listAttribute&&listAttribute.length&&(c+="&list_file="+listFile+"&list_attribute="+listAttribute);lastAjaxUpdate=(new Date).getTime();$.get(c,function(c){try{var d=document.createRange().createContextualFragment(JSON.parse(c).html);b in elements||
(a.add([d.firstChild]),elements[b]=JSON.parse(c).element,saveElements(a));applyFormatting();showOrHideElements();$("div#element_"+b+" div.item-content").css("visibility","visible");updateDashboardName(JSON.parse(c).dashboard_name);$("#delete_dashboard").css("display","inline")}catch(g){console.log(g.message)}})}
