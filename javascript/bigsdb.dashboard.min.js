...
$(function(){var a=$("#layout").val(),c=$("#fill_gaps").prop("checked");try{var b=(new Muuri(".grid",{dragEnabled:!0,layout:{alignRight:a.includes("right"),alignBottom:a.includes("bottom"),fillGaps:c}})).on("move",function(){saveLayout(b)});order&&loadLayout(b,order)}catch(d){console.log(d.message)}$("#panel_trigger,#close_trigger").click(function(){$("#modify_panel").toggle("slide",{direction:"right"},"fast");$("#panel_trigger").show();return!1});$("#panel_trigger").show();$("#layout").change(function(){a=
$("#layout").val();try{b._settings.layout.alignRight=a.includes("right"),b._settings.layout.alignBottom=a.includes("bottom"),b.layout()}catch(d){}$.ajax(ajax_url+"&attribute=layout&value="+a)});$("#fill_gaps").change(function(){c=$("#fill_gaps").prop("checked");try{b._settings.layout.fillGaps=c,b.layout()}catch(d){}$.ajax(ajax_url+"&attribute=fill_gaps&value="+(c?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(ajax_url+"&attribute=edit_elements&value="+
(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(ajax_url+"&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$(".dashboard_edit_element").click(function(){var a=$(this).attr("data-id");$("span#control_"+a).hide();$("span#wait_"+a).show();$.get(modal_control_url+"&control="+a,function(b){$(b).appendTo("body").modal();
$("span#control_"+a).show();$("span#wait_"+a).hide()})});$(".dashboard_remove_element").click(function(){var a=$(this).attr("data-id"),c=b.getItem($("div#element_"+a)[0]);b.remove([c],{removeElements:!0});delete elements[a];saveElements(b);0==Object.keys(elements).length&&$("div#dashboard").html(empty)});["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(c){c=$(this).attr("id").replace("_"+a,"");changeElementDimension(b,c,a)})});$("a#dashboard_toggle").on("click",
function(){$.get(ajax_url+"&attribute=default&value=0",function(){window.location=url+"?db="+instance})})});
