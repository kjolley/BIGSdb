...
$("#layout").val();try{c._settings.layout.alignRight=a.includes("right"),c._settings.layout.alignBottom=a.includes("bottom"),c.layout()}catch(d){}$.ajax(ajax_url+"&attribute=layout&value="+a)});$("#fill_gaps").change(function(){b=$("#fill_gaps").prop("checked");try{c._settings.layout.fillGaps=b,c.layout()}catch(d){}$.ajax(ajax_url+"&attribute=fill_gaps&value="+(b?1:0))});$("#edit_elements").change(function(){var a=$("#edit_elements").prop("checked");$.ajax(ajax_url+"&attribute=edit_elements&value="+
(a?1:0));$("span.dashboard_edit_element").css("display",a?"inline":"none")});$("#remove_elements").change(function(){var a=$("#remove_elements").prop("checked");$.ajax(ajax_url+"&attribute=remove_elements&value="+(a?1:0));$("span.dashboard_remove_element").css("display",a?"inline":"none")});$("#add_element").click(function(){var a=getNextid();console.log(a);addElement(c,a)});$("div#dashboard").on("click",".dashboard_edit_element",function(){var a=$(this).attr("data-id");editElement(c,a)});$("div#dashboard").on("click",
".dashboard_remove_element",function(){var a=$(this).attr("data-id");removeElement(c,a)});["width","height"].forEach(function(a){$(document).on("change","."+a+"_select",function(b){b=$(this).attr("id").replace("_"+a,"");changeElementDimension(c,b,a)})});$("a#dashboard_toggle").on("click",function(){$.get(ajax_url+"&attribute=default&value=0",function(){window.location=url+"?db="+instance})})});
function getNextid(){return 0===Object.keys(elements).length?1:Math.max.apply(Math,[].concat($jscomp.arrayFromIterable(Object.keys(elements))))+1}function addElement(a,b){0===Object.keys(elements).length&&$("div#dashboard").html("");$.get(url+"?db="+instance+"&page=dashboard&new="+b,function(c){var d=document.createRange().createContextualFragment(JSON.parse(c).html);a.add([d.firstChild]);elements[b]=JSON.parse(c).element;saveElements(a)})}
