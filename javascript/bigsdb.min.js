...
"Enable tooltips");$("dt").each(function(){this.offsetWidth+1<this.scrollWidth&&($(this).prop("title",$(this).text()),$(this).tooltip(),$(this).css("cursor","pointer"))});$(window).resize(function(){show_expand_trigger()});window.jQuery&&$.fn.select2&&$("select:not(.locuslist):not(.widelist)").not(".select2-hidden-accessible").select2({minimumResultsForSearch:0});$(document).on("select2:open",function(){document.querySelector(".select2-search__field").focus()});$(".select2-selection__choice").removeAttr("title");
$(".select2-selection__rendered").removeAttr("title");$(document).on("keydown",".select2-selection",function(a){if(1===a.key.length&&!a.ctrlKey&&!a.altKey&&!a.metaKey){var c=$(this).closest(".select2-container").prev("select");c.one("select2:open",function(){var b=document.querySelector(".select2-container--open .select2-search__field");b.value=a.key;$(b).trigger("input")});c.select2("open");a.preventDefault()}})});
$.urlParam=function(a){a=(new RegExp("[?&]"+a+"=([^&#]*)")).exec(window.location.href);return null==a?null:a[1]||0};function set_page_width(){$("span#expand").is(":visible")?($("div.main_content").css({"max-width":max_width+"px"}),$("div#title_container").css({"max-width":max_width-15+"px"})):($("div.main_content").css({"max-width":"calc(100vw - 40px)"}),$("div#title_container").css({"max-width":"calc(100vw - 15px)"}))}
function show_expand_trigger(){$(window).width()>max_width+100?$("a#expand_trigger").show():$("a#expand_trigger").hide()}function reloadTooltips(){var a=$("a[title],span[title]").not(".lightbox");$.each(a,function(a,b){b=$(this).attr("title");b=b.replace(/^([^<h3>].+?) - /,"<h3>$1</h3>");$(this).tooltip({content:b})});a=$("label[title]").not(".lightbox");$.each(a,function(a,b){b=$(this).attr("title");$(this).tooltip({content:b})})}
