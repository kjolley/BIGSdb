...
"Enable tooltips");$("dt").each(function(){this.offsetWidth+1<this.scrollWidth&&($(this).prop("title",$(this).text()),$(this).tooltip(),$(this).css("cursor","pointer"))});$(window).resize(function(){show_expand_trigger()});$(document).on("select2:open",function(){document.querySelector(".select2-search__field").focus()});$(document).on("keydown",".select2-selection",function(a){if(1===a.key.length&&!a.ctrlKey&&!a.altKey&&!a.metaKey){var c=$(this).closest(".select2-container").prev("select");c.one("select2:open",
function(){var b=document.querySelector(".select2-container--open .select2-search__field");b.value=a.key;$(b).trigger("input")});c.select2("open");a.preventDefault()}})});$.urlParam=function(a){a=(new RegExp("[?&]"+a+"=([^&#]*)")).exec(window.location.href);return null==a?null:a[1]||0};
