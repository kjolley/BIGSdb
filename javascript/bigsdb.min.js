...
($("div.main_content").css({"max-width":"calc(100vw - 40px)"}),$("span#expand, span#expand_label_expand").hide(),$("span#contract, span#expand_label_contract").show()):($("div.main_content").css({"max-width":max_width+"px"}),$("span#expand, span#expand_label_expand").show(),$("span#contract, span#expand_label_contract").hide())});reloadTooltips();$("a#toggle_tooltips").prop("title",$("a#toggle_tooltips").hasClass("tooltips_enabled")?"Disable tooltips":"Enable tooltips");$("dt").each(function(){this.offsetWidth+
1<this.scrollWidth&&($(this).prop("title",$(this).text()),$(this).tooltip(),$(this).css("cursor","pointer"))});$(window).resize(function(){show_expand_trigger()})});$.urlParam=function(a){a=(new RegExp("[?&]"+a+"=([^&#]*)")).exec(window.location.href);return null==a?null:a[1]||0};function show_expand_trigger(){$(window).width()>max_width+100?$("a#expand_trigger").show():$("a#expand_trigger").hide()}
