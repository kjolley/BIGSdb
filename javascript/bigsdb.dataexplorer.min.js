...
success:function(a){$("div#waiting").css("display","none");a=JSON.parse(a);d3.select("div#tree").select("svg").remove();loadTree(a)}})}
...
a.y0+","+a.x0+")"}).attr("fill-opacity",0).attr("stroke-opacity",0);var n=d3.select("div#tooltip");h.append("circle").attr("r",5).attr("fill",function(b){return b._children?"#555":"#999"}).attr("stroke-width",10).attr("cursor","pointer").on("click",function(b,a){a.children=a.children?null:a._children;d(a)}).on("mouseover",function(b,a){null!=a._children&&n.html("Click to expand/contract").style("visibility","visible")}).on("mouseout",function(){n.style("visibility","hidden")});h.append("text").attr("dy",
"0.31em").attr("x",function(b){return b._children?-12:12}).attr("text-anchor",function(b){return b._children?"end":"start"}).text(function(b){return b.data.value}).attr("cursor","pointer").on("click touchstart",function(b,a){window.open(a.data.url)}).on("mouseover",function(a,c){n.html("Click to see records ("+c.data.count+")").style("visibility","visible")}).on("mouseout",function(){n.style("visibility","hidden")}).clone(!0).lower().attr("stroke-linejoin","round").attr("stroke-width",3).attr("stroke",
"white");h.filter(function(a){return a.parent}).append("rect").attr("width",function(a){return a.parent?100*a.data.count/a.parent.data.count:0}).attr("x",function(a){return a._children?-(100*a.data.count/a.parent.data.count)-12:12}).attr("y",function(a){return 6}).attr("height",function(a){return a.parent?1:0}).attr("stroke","blue");e.merge(h).transition(g).attr("transform",function(a){return"translate("+a.y+","+a.x+")"}).attr("fill-opacity",1).attr("stroke-opacity",1);e.exit().transition(g).remove().attr("transform",
function(b){return"translate("+a.y+","+a.x+")"}).attr("fill-opacity",0).attr("stroke-opacity",0);k=v.selectAll("path").data(k,function(a){return a.target.id});e=k.enter().append("path").attr("d",function(b){b={x:a.x0,y:a.y0};return r({source:b,target:b})});k.merge(e).transition(g).attr("d",r);k.exit().transition(g).remove().attr("d",function(b){b={x:a.x,y:a.y};return r({source:b,target:b})});c.eachBefore(function(a){a.x0=a.x;a.y0=a.y})}var c=d3.hierarchy(a.hierarchy);a=a.fields.cleaned.length;var f=
{top:10,right:120,bottom:10,left:40},p=d3.select("div#tree").node().getBoundingClientRect().width;a=p/(a+1);var t=d3.tree().nodeSize([20,a]),r=d3.linkHorizontal().x(function(a){return a.y}).y(function(a){return a.x});c.x0=a/2;c.y0=0;c.descendants().forEach(function(a,c){a.id=c;a._children=a.children;0<a.depth&&(a.children=null)});var m=d3.select("div#tree").append("svg").attr("viewBox",[-f.left,-f.top,p,20]).style("font","12px sans-serif").style("user-select","none"),v=m.append("g").attr("fill","none").attr("stroke",
"#555").attr("stroke-opacity",.4).attr("stroke-width",1.5),u=m.append("g");d(c)}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
