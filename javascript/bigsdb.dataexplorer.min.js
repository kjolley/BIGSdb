...
c){return d3.descending(a.data.count,c.data.count)});console.log(c);a=a.fields.cleaned.length;var f={top:10,right:120,bottom:10,left:40},p=d3.select("div#tree").node().getBoundingClientRect().width;a=p/(a+1);var t=d3.tree().nodeSize([20,a]),r=d3.linkHorizontal().x(function(a){return a.y}).y(function(a){return a.x});c.x0=a/2;c.y0=0;c.descendants().forEach(function(a,c){a.id=c;a._children=a.children;0<a.depth&&(a.children=null)});var m=d3.select("div#tree").append("svg").attr("viewBox",[-f.left,-f.top,
p,20]).style("font","13px sans-serif").style("user-select","none"),v=m.append("g").attr("fill","none").attr("stroke","#555").attr("stroke-opacity",.4).attr("stroke-width",1.5),u=m.append("g");d(c)}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
