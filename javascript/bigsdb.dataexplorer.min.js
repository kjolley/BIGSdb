...
$("#"+a).switchClass("expandable_expanded","expandable_retracted data_explorer",1E3,"easeInOutQuad",function(){$("#expand_"+a).html('<span class="fas fa-chevron-down"></span>')}):$("#"+a).switchClass("expandable_retracted data_explorer","expandable_expanded",1E3,"easeInOutQuad",function(){$("#expand_"+a).html('<span class="fas fa-chevron-up"></span>')})})});
function reloadTable(){$("div#waiting").css("display","block");var a=$("#include_old_versions").is(":checked")?1:0;$.ajax({url:url+"&page=explorer&updateTable=1&field="+field+"&record_age="+recordAge+"&include_old_versions="+a}).done(function(a){$("div#table_div").html(a);$(".tablesorter").tablesorter({widgets:["zebra"]});$("div#waiting").css("display","none");a=(a.match(/value_row/g)||[]).length;$("span#unique_values").html(a);var b=0;$("td.value_count").each(function(){var a=$(this).html().replace(",",
"");b+=parseInt(a,10)||0});$("span#total_records").html(commify(b))})}function commify(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};
