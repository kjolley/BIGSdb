...
"inline");d3.select("#map").selectAll("path").attr("fill","none");d3.select("#map").selectAll("path").attr("stroke","#444");d3.select("#map").selectAll(".background").attr("fill","none");var a=d3.select("svg").attr("xmlns","http://www.w3.org/2000/svg").node().parentNode.innerHTML;a=a.replace(/<\/svg>.*$/,"</svg>");a=new Blob([a],{type:"image/svg+xml"});var b=$("#field").val().replace(/^.+\.\./,"").replace(/^af_/,"").replace(/___/g,"_")+".svg";saveAs(a,b)});$("#expand_themes").click(function(){$("#themes_shown").toggle();
$("#themes_hidden").toggle();$("#themes_hidden").is(":visible")?$("#expanded_themes").hide(500):$("#expanded_themes").show(500)})});function load_map_after_prefs_loaded(a,b){prefs_load_attempts++;if(!prefs_loaded&&200>prefs_load_attempts)return setTimeout(function(){load_map_after_prefs_loaded(a,b)},50);load_map(a,b)}
