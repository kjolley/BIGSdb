...
function load_geography(a,b){$("#bb_chart").html("");$("#geography").css("height","400px");var c=[],d=new ol.layer.Tile({visible:!0,source:new ol.source.OSM({crossOrigin:null})}),e=new ol.layer.Tile({visible:!1,source:new ol.source.XYZ({attributions:["Powered by Esri;","Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community"],attributionsCollapsible:!0,url:"https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",maxZoom:23})});c.push(d);console.log(use_arcgis);
use_arcgis&&c.push(e);d3.json(a).then(function(a){var d=new ol.Map({target:"geography",layers:c,view:new ol.View({center:ol.proj.fromLonLat([0,20]),zoom:2,minZoom:1,maxZoom:16})}),f=get_marker_layer(a);d.addLayer(f);f.getSource().getFeatures().length&&d.getView().fit(f.getSource().getExtent(),{size:d.getSize(),padding:[10,10,10,10],minZoom:2,maxZoom:12,constrainResolution:!1});d.on("postrender",function(e){b===selected_field&&($("#map").html(""),$("div#waiting").css("display","none"),$("#geography_controls").css("display",
"block"),$("#bb_chart").css("min-height",0),show_export_options(),$("input[name='geography_view']").off("change").change(function(){"Aerial"==$("input[name='geography_view']:checked").val()?(c[0].setVisible(!1),c[1].setVisible(!0)):(c[0].setVisible(!0),c[1].setVisible(!1));set_prefs("map_style",$("input[name='geography_view']:checked").val())}),$(".marker_colour").off("click").click(function(){set_prefs("marker_colour",this.id);marker_colour=this.id;d.removeLayer(f);f=get_marker_layer(a);d.addLayer(f)}),
$("#marker_size").off("slidechange").on("slidechange",function(){marker_size=$("#marker_size").slider("value");set_prefs("marker_size",marker_size);d.removeLayer(f);f=get_marker_layer(a);d.addLayer(f)}))});$("#marker_size").slider({min:0,max:10,value:marker_size});d.on("rendercomplete",function(a){a=document.querySelector("canvas");var b=document.getElementById("export_map_image");b.setAttribute("download",selected_field+"_map.png");b.setAttribute("href",a.toDataURL("image/png"))})})}
