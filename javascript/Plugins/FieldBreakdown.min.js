...
"Conic Equal Area":d3.geoConicEqualArea,Equirectangular:d3.geoEquirectangular,"Natural Earth":d3.geoNaturalEarth,Mercator:d3.geoMercator,Robinson:d3.geoRobinson,Stereographic:d3.geoStereographic,Times:d3.geoTimes,"Transverse Mercator":d3.geoTransverseMercator,"Winkel tripel":d3.geoWinkel3},k=f[theme];d3.json(a).then(function(l){"country"==b&&(l=merge_terms(l));var n=get_range(l),m=d3.geomap.choropleth().geofile(h).colors(k).column("value").format(d3.format(",d")).legend({width:50,height:120}).projection(g[projection]).duration(1E3).domain([0,
...
set_prefs("theme",this.id);theme=this.id});$("#projection").off("change").change(function(){d3.selectAll("svg").remove();projection=$("#projection").val();m.projection(g[projection]).draw(p);set_prefs("projection",projection)})})}function merge_terms(a){for(var b={},c=0;c<a.length;c++)b[a[c].iso3]="undefined"==typeof b[a[c].iso3]?a[c].value:b[a[c].iso3]+a[c].value;a=[];var d=Object.keys(b);for(c=0;c<d.length;c++)a.push({iso3:d[c],value:b[d[c]]});return a}
var delay=function(){var a=0;return function(b,c){clearTimeout(a);a=setTimeout(b,c)}}();function get_range(a){var b=a.length;b=parseInt((10<b?.05:.25)*b);for(var c=10,d,e,h=[],f;;){for(var g=[1,2,5],k=0;k<g.length;k++){d=g[k]*c;h.push(d);if(e&&5<=h.length){f=1;break}for(var l=4*d/5,n=0,m=0;m<a.length;m++)a[m].value>=l&&n++;n<=b&&!e&&(e=d)}if(f)break;c*=10;if(1E7<d){e||(e=d);break}}return{recommended:e,options:h.slice(-5)}}
...
function pie_json_to_cols(a,b){var c=[],d=[],e=0,h=0,f=0;a.forEach(function(a){a.label=a.label.toString();e++;e>=b?(h+=a.value,f++,e==b&&(d=[a.label,a.value])):c.push([a.label,a.value])});1==f?c.push(d):1<f&&c.push(["Others",h]);return{columns:c,count:e}}
function load_line(a,b,c){$("#bar_controls").css("display","none");$("#pie_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");$(".transform_to_map").css("display","none");$("#line_height").off("slidechange");var d=b.replace(/^.+\.\./,"");d3.json(a).then(function(e){remove_null(e);var h=["value"],f=["date"],g=0,k=0;e.forEach(function(a){g++;f.push(a.label);c?(k+=a.value,h.push(k)):h.push(a.value)});d+=" ("+g+" value"+(1==g?"":"s")+
...
function load_geography(a,b){$("#bb_chart").html("");$("#geography").css("height","400px");var c=["RoadOnDemand","AerialWithLabelsOnDemand"],d=["Map","Aerial"],e=[];if(bingmaps_api){var h=$("input[name='geography_view']:checked").val(),f;var g=0;for(f=c.length;g<f;++g)e.push(new ol.layer.Tile({visible:d[g]==h?!0:!1,preload:Infinity,source:new ol.source.BingMaps({key:bingmaps_api,imagerySet:c[g]})}))}else e.push(new ol.layer.Tile({source:new ol.source.OSM({crossOrigin:null})}));d3.json(a).then(function(a){var c=
...
function(){marker_size=$("#marker_size").slider("value");set_prefs("marker_size",marker_size);c.removeLayer(d);d=get_marker_layer(a);c.addLayer(d)}))});$("#marker_size").slider({min:1,max:10,value:marker_size})})}
