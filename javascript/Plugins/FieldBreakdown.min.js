...
function load_geography(a,b){$("#bb_chart").html("");$("#geography").css("height","400px");var c=$("input[name='geography_view']:checked").val();"undefined"==typeof c&&(c="Map");var d=get_ol_layers(mapping_option,c);d3.json(a).then(function(a){var c=new ol.Map({target:"geography",layers:d,view:new ol.View({center:ol.proj.fromLonLat([0,20]),zoom:2,minZoom:1,maxZoom:16})}),f=get_marker_layer(a);c.addLayer(f);f.getSource().getFeatures().length&&c.getView().fit(f.getSource().getExtent(),{size:c.getSize(),
padding:[10,10,10,10],minZoom:2,maxZoom:12,constrainResolution:!1});c.on("postrender",function(g){b===selected_field&&($("#map").html(""),$("div#waiting").css("display","none"),$("#geography_controls").css("display","block"),$("#bb_chart").css("min-height",0),show_export_options(),$("input[name='geography_view']").off("change").change(function(){"Aerial"==$("input[name='geography_view']:checked").val()?(d[0].setVisible(!1),d[1].setVisible(!0),"undefined"!==typeof d[2]&&d[2].setVisible(!0),1==mapping_option&&
$("a#maptiler_logo").show()):(d[0].setVisible(!0),d[1].setVisible(!1),"undefined"!==typeof d[2]&&d[2].setVisible(!1),1==mapping_option&&$("a#maptiler_logo").hide());set_prefs("map_style",$("input[name='geography_view']:checked").val())}),$(".marker_colour").off("click").click(function(){set_prefs("marker_colour",this.id);marker_colour=this.id;c.removeLayer(f);f=get_marker_layer(a);c.addLayer(f)}),$("#marker_size").off("slidechange").on("slidechange",function(){marker_size=$("#marker_size").slider("value");
set_prefs("marker_size",marker_size);c.removeLayer(f);f=get_marker_layer(a);c.addLayer(f)}))});$("#marker_size").slider({min:0,max:10,value:marker_size});c.on("rendercomplete",function(a){a=document.querySelector("canvas");var b=document.getElementById("export_map_image");b.setAttribute("download",selected_field+"_map.png");b.setAttribute("href",a.toDataURL("image/png"))})})}
