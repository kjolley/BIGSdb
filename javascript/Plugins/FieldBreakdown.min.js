...
function load_pie(a,b,c){$("#bar_controls").css("display","none");$("#line_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");if("undefined"!=typeof b){var d=b.replace(/^.+\.\./,"");"loci"==get_field_type()&&"undefined"!==typeof locus_labels&&void 0!=locus_labels[b]&&(d=locus_labels[b]);d=d.replace(/^s_\d+_/,"");var f=d3.format(".1f");d3.json(a).then(function(g){for(var h=pie_json_to_cols(g,c),e=pie_json_to_cols(g,50),m=0,k=0;k<
e.columns.length;k++)"Others"==e.columns[k][0]&&(m=1);m||e.columns.push(["Others",0]);d+=" ("+h.count+" value"+(1==h.count?"":"s")+")";var l=bb.generate({bindto:"#bb_chart",title:{text:d},data:{columns:e.columns,names:e.names,type:pie?"pie":"donut",order:null,colors:{Others:"#aaa"}},pie:{label:{show:!0},expand:!1},legend:{show:!0,position:"bottom"},size:{height:500},tooltip:{format:{value:function(a,b,c){return a+" ("+f(100*b)+"%)"}}}});l.unload();l.load({columns:h.columns});$("#segments").on("slidechange",
...
function pie_json_to_cols(a,b){var c=[],d={},f=[],g=0,h=0,e=0;a.forEach(function(a){a.label=a.label.toString();""==a.label&&(a.label="No value");g++;var k="Data"+g;g>=b?(h+=a.value,e++,g==b&&(f=[a.label,a.value])):c.push([k,a.value]);d[k]=a.label});1==e?c.push(f):1<e&&c.push(["Others",h]);return{columns:c,names:d,count:g}}
function load_line(a,b,c){$("#bar_controls").css("display","none");$("#pie_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");$(".transform_to_map").css("display","none");$("#line_height").off("slidechange");var d=b.replace(/^.+\.\./,"");d3.json(a).then(function(f){remove_null(f);var g=["value"],h=["date"],e=0,m=0;f.forEach(function(a){e++;h.push(a.label);c?(m+=a.value,g.push(m)):g.push(a.value)});d+=" ("+e+" value"+(1==e?"":"s")+
...
function load_bar_json(a,b,c){remove_null(a);$("#line_controls").css("display","none");$("#pie_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");$("#bar_height").off("slidechange");b=b.replace(/^.+\.\./,"");var d=Object.keys(a).length,f=bb.generate({bindto:"#bb_chart",title:{text:b+(" ("+d+" value"+(1==d?"":"s")+")")},data:{json:a,keys:{x:"label",value:["value"]},type:"bar"},bar:{width:{ratio:.7}},axis:{rotated:c,x:{type:"category",
tick:{culling:!0},height:100}},legend:{show:!1},padding:{right:20}});f.resize({height:height});$("#bar_height").slider({min:300,max:800,value:height});$("#bar_controls").css("display","block");$("#bar_height").on("slidechange",function(){height=$("#bar_height").slider("value");f.resize({height:height});set_prefs("height",height)});show_export_options()}
...
function load_geography(a,b){$("#bb_chart").html("");$("#geography").css("height","400px");var c=$("input[name='geography_view']:checked").val();"undefined"==typeof c&&(c="Map");var d=get_ol_layers(mapping_option,c);d3.json(a).then(function(a){var g=new ol.control.Attribution({collapsible:3>mapping_option?!1:!0}),h=new ol.Map({target:"geography",controls:ol.control.defaults({attribution:!1}).extend([g]),layers:d,view:new ol.View({center:ol.proj.fromLonLat([0,20]),zoom:2,minZoom:1,maxZoom:16})}),e=
get_marker_layer(a);h.addLayer(e);e.getSource().getFeatures().length&&h.getView().fit(e.getSource().getExtent(),{size:h.getSize(),padding:[10,10,10,10],minZoom:2,maxZoom:12,constrainResolution:!1});h.on("postrender",function(c){b===selected_field&&($("#map").html(""),$("div#waiting").css("display","none"),$("#geography_controls").css("display","block"),$("#bb_chart").css("min-height",0),show_export_options(),$("input[name='geography_view']").off("change").change(function(){"Aerial"==$("input[name='geography_view']:checked").val()?
(d[0].setVisible(!1),d[1].setVisible(!0),"undefined"!==typeof d[2]&&d[2].setVisible(!0),g.setCollapsible(!0),g.setCollapsed(!0)):(d[0].setVisible(!0),d[1].setVisible(!1),"undefined"!==typeof d[2]&&d[2].setVisible(!1),3>mapping_option&&(g.setCollapsible(!1),g.setCollapsed(!1)));display_maptiler_logo();set_prefs("map_style",$("input[name='geography_view']:checked").val())}),display_maptiler_logo(),$(".marker_colour").off("click").click(function(){set_prefs("marker_colour",this.id);marker_colour=this.id;
h.removeLayer(e);e=get_marker_layer(a);h.addLayer(e)}),$("#marker_size").off("slidechange").on("slidechange",function(){marker_size=$("#marker_size").slider("value");set_prefs("marker_size",marker_size);h.removeLayer(e);e=get_marker_layer(a);h.addLayer(e)}))});$("#marker_size").slider({min:0,max:10,value:marker_size});h.on("rendercomplete",function(a){a=document.querySelector("canvas");var b=document.getElementById("export_map_image");b.setAttribute("download",selected_field+"_map.png");b.setAttribute("href",
a.toDataURL("image/png"))});3>mapping_option&&(g.setCollapsible("Map"==c?!1:!0),g.setCollapsed("Map"==c?!1:!0))})}
