...
function load_map(a,b){$("#bar_height").off("slidechange");var c=$("#map").width();$("#bb_chart").html("");var d="country"==b?"iso3":"continent",e="country"==b?"units":"continents",h="country"==b?"/javascript/topojson/countries.json":"/javascript/topojson/continents.json",f={theme_grey:colorbrewer.Greys[5],theme_blue:colorbrewer.Blues[5],theme_green:colorbrewer.Greens[5],theme_purple:colorbrewer.Purples[5],theme_orange:colorbrewer.Oranges[5],theme_red:colorbrewer.Reds[5],theme_blue_green:colorbrewer.BuGn[5],
theme_blue_purple:colorbrewer.BuPu[5],theme_green_blue:colorbrewer.GnBu[5],theme_orange_red:colorbrewer.OrRd[5],theme_purple_blue:colorbrewer.PuBu[5],theme_purple_blue_green:colorbrewer.PuBuGn[5],theme_purple_red:colorbrewer.PuRd[5],theme_red_purple:colorbrewer.RdPu[5],theme_yellow_green:colorbrewer.YlGn[5],theme_yellow_green_blue:colorbrewer.YlGnBu[5],theme_yellow_orange_brown:colorbrewer.YlOrBr[5],theme_yellow_orange_red:colorbrewer.YlOrRd[5]},g={"Azimuthal Equal Area":d3.geoAzimuthalEqualArea,
"Conic Equal Area":d3.geoConicEqualArea,Equirectangular:d3.geoEquirectangular,"Natural Earth":d3.geoNaturalEarth,Mercator:d3.geoMercator,Robinson:d3.geoRobinson,Stereographic:d3.geoStereographic,Times:d3.geoTimes,"Transverse Mercator":d3.geoTransverseMercator,"Winkel tripel":d3.geoWinkel3},m=f[theme];d3.json(a).then(function(l){"country"==b&&(l=merge_terms(l));var k=get_range(l),n=d3.geomap.choropleth().geofile(h).colors(m).column("value").format(d3.format(",d")).legend({width:50,height:120}).projection(g[projection]).duration(1E3).domain([0,
k.recommended]).valueScale(d3.scaleQuantize).unitId(d).units(e).postUpdate(function(){finished_map(a,b)}),p=d3.select("#map").datum(l);n.draw(p);$(window).resize(function(){b===selected_field&&delay(function(){c!=$("#map").width()&&(c=$("#map").width(),$("#map").html(""),load_map(a,b))},500)});$("#colour_range").slider({min:0,max:4,value:k.options.indexOf(k.recommended),slide:function(a,b){n.domain([0,k.options[b.value]]).duration(0).update()}});$(".theme").off("click").click(function(){n.colors(f[this.id]).duration(0).update();
set_prefs("theme",this.id);theme=this.id});$("#projection").off("change").change(function(){d3.selectAll("svg").remove();projection=$("#projection").val();n.projection(g[projection]).draw(p);set_prefs("projection",projection)})})}function merge_terms(a){for(var b={},c=0;c<a.length;c++)b[a[c].iso3]="undefined"==typeof b[a[c].iso3]?a[c].value:b[a[c].iso3]+a[c].value;a=[];var d=Object.keys(b);for(c=0;c<d.length;c++)a.push({iso3:d[c],value:b[d[c]]});return a}
var delay=function(){var a=0;return function(b,c){clearTimeout(a);a=setTimeout(b,c)}}();function get_range(a){var b=a.length;b=parseInt((10<b?.05:.25)*b);for(var c=10,d,e,h=[],f;;){for(var g=[1,2,5],m=0;m<g.length;m++){d=g[m]*c;h.push(d);if(e&&5<=h.length){f=1;break}for(var l=4*d/5,k=0,n=0;n<a.length;n++)a[n].value>=l&&k++;k<=b&&!e&&(e=d)}if(f)break;c*=10;if(1E7<d){e||(e=d);break}}return{recommended:e,options:h.slice(-5)}}
...
function pie_json_to_cols(a,b){var c=[],d={},e=[],h=0,f=0,g=0;a.forEach(function(a){a.label=a.label.toString();""==a.label&&(a.label="No value");h++;var m="Data"+h;h>=b?(f+=a.value,g++,h==b&&(e=[a.label,a.value])):c.push([m,a.value]);d[m]=a.label});1==g?c.push(e):1<g&&c.push(["Others",f]);return{columns:c,names:d,count:h}}
function load_line(a,b,c){$("#bar_controls").css("display","none");$("#pie_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");$(".transform_to_map").css("display","none");$("#line_height").off("slidechange");var d=b.replace(/^.+\.\./,"");d3.json(a).then(function(e){remove_null(e);var h=["value"],f=["date"],g=0,m=0;e.forEach(function(a){g++;f.push(a.label);c?(m+=a.value,h.push(m)):h.push(a.value)});d+=" ("+g+" value"+(1==g?"":"s")+
")";var l=bb.generate({bindto:"#bb_chart",title:{text:d},data:{x:"date",columns:[f,h],type:line?"line":"bar",order:"asc"},axis:{x:{type:"timeseries",tick:{format:"%Y-%m-%d",count:100,rotate:90,fit:!0},height:100}},legend:{show:!1}});l.resize({height:height});$(".transform_to_bar").off("click").click(function(){l.unload();load_line(a,b,0);$(".transform_to_bar").css("display","none");$(".transform_to_line").css("display","inline");line=0});$(".transform_to_line").off("click").click(function(){l.unload();
...
function load_bar(a,b,c){d3.json(a).then(function(a){load_bar_json(a,b,c);$("div#waiting").css("display","none")},function(a){console.log(a);$("#bb_chart").html('<p style="text-align:center;margin-top:5em"><span class="error_message">Error accessing data.</span></p>')});$('input[name="orientation"]').on("change",function(){var b=$("#field").val();c=is_vertical();load_bar(a,b,c)})}function get_osm_layer(a){return new ol.layer.Tile({visible:a,source:new ol.source.OSM({crossOrigin:null})})}
function get_maptiler_layer(a){return new ol.layer.Tile({visible:a,source:new ol.source.TileJSON({attributions:'<a href="https://www.maptiler.com/copyright/" target="_blank">&copy; MapTiler</a>',attributionsCollapsible:!1,url:"https://api.maptiler.com/maps/hybrid/tiles.json?key="+maptiler_key,tileSize:512,maxZoom:20})})}
function get_arcgis_world_imagery_layer(a){return new ol.layer.Tile({visible:a,source:new ol.source.XYZ({attributions:["Powered by Esri;","Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community"],attributionsCollapsible:!0,url:"https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",maxZoom:23})})}
function get_arcgis_hybdrid_ref_layer(a){return new ol.layer.VectorTile({visible:a,source:new ol.source.VectorTile({format:new ol.format.MVT({layers:["Boundary line","Admin0 point","City small scale","City large scale"]}),url:"https://basemaps.arcgis.com/arcgis/rest/services/World_Basemap_v2/VectorTileServer/tile/{z}/{y}/{x}.pbf"}),style:function(a,c){if("Admin0 point"==a.get("layer"))return new ol.style.Style({text:new ol.style.Text({text:a.get("_name"),font:"14px sans-serif",fill:new ol.style.Fill({color:"#aaa"}),
stroke:new ol.style.Stroke({color:"#000",width:3})})});if("City small scale"==a.get("layer")||"City large scale"==a.get("layer"))return new ol.style.Style({text:new ol.style.Text({text:a.get("_name"),font:"11px sans-serif",fill:new ol.style.Fill({color:"#aaa"}),stroke:new ol.style.Stroke({color:"#000",width:2}),offsetY:10}),image:new ol.style.Circle({radius:3,fill:new ol.style.Fill({color:"#fff"}),stroke:new ol.style.Stroke({color:"#000",width:1})})});if("Boundary line"==a.get("layer"))return new ol.style.Style({stroke:new ol.style.Stroke({color:"#aaa",
width:1})})}})}function get_arcgis_world_streetmap_layer(a){return new ol.layer.Tile({visible:a,source:new ol.source.XYZ({attributions:["Powered by Esri;","Source: Esri, HERE, Garmin, USGS, Intermap, INCREMENT P, NRCAN, Esri Japan, METI, Esri China (Hong Kong), NOSTRA, &copy; OpenStreetMap contributors, and the GIS User Community"],attributionsCollapsible:!0,url:"https://services.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}",maxZoom:23})})}
function load_geography(a,b){$("#bb_chart").html("");$("#geography").css("height","400px");var c=$("input[name='geography_view']:checked").val();"undefined"==typeof c&&(c="Map");var d=[];switch(mapping_option){case 0:d.push(get_osm_layer("Map"===c?!0:!1));break;case 1:d.push(get_osm_layer("Map"===c?!0:!1));d.push(get_maptiler_layer("Aerial"===c?!0:!1));"Aerial"===c&&$("a#maptiler_logo").show();break;case 2:d.push(get_osm_layer("Map"===c?!0:!1));d.push(get_arcgis_world_imagery_layer("Aerial"===c?!0:
!1));d.push(get_arcgis_hybdrid_ref_layer("Aerial"===c?!0:!1));break;case 3:d.push(get_arcgis_world_streetmap_layer("Map"===c?!0:!1)),d.push(get_arcgis_world_imagery_layer("Aerial"===c?!0:!1)),d.push(get_arcgis_hybdrid_ref_layer("Aerial"===c?!0:!1))}d3.json(a).then(function(a){var c=new ol.Map({target:"geography",layers:d,view:new ol.View({center:ol.proj.fromLonLat([0,20]),zoom:2,minZoom:1,maxZoom:16})}),f=get_marker_layer(a);c.addLayer(f);f.getSource().getFeatures().length&&c.getView().fit(f.getSource().getExtent(),
{size:c.getSize(),padding:[10,10,10,10],minZoom:2,maxZoom:12,constrainResolution:!1});c.on("postrender",function(g){b===selected_field&&($("#map").html(""),$("div#waiting").css("display","none"),$("#geography_controls").css("display","block"),$("#bb_chart").css("min-height",0),show_export_options(),console.log(d.length),$("input[name='geography_view']").off("change").change(function(){"Aerial"==$("input[name='geography_view']:checked").val()?(d[0].setVisible(!1),d[1].setVisible(!0),"undefined"!==
typeof d[2]&&d[2].setVisible(!0),1==mapping_option&&$("a#maptiler_logo").show()):(d[0].setVisible(!0),d[1].setVisible(!1),"undefined"!==typeof d[2]&&d[2].setVisible(!1),1==mapping_option&&$("a#maptiler_logo").hide());set_prefs("map_style",$("input[name='geography_view']:checked").val())}),$(".marker_colour").off("click").click(function(){set_prefs("marker_colour",this.id);marker_colour=this.id;c.removeLayer(f);f=get_marker_layer(a);c.addLayer(f)}),$("#marker_size").off("slidechange").on("slidechange",
function(){marker_size=$("#marker_size").slider("value");set_prefs("marker_size",marker_size);c.removeLayer(f);f=get_marker_layer(a);c.addLayer(f)}))});$("#marker_size").slider({min:0,max:10,value:marker_size});c.on("rendercomplete",function(a){a=document.querySelector("canvas");var b=document.getElementById("export_map_image");b.setAttribute("download",selected_field+"_map.png");b.setAttribute("href",a.toDataURL("image/png"))})})}
...
a.label.match(/(\-?\d+\.?\d*),\s*(\-?\d+\.?\d*)/);if(null!=b){var d=parseFloat(b[1]);b=parseFloat(b[2]);for(var e,g=0;9>g;++g)if(parseInt(a.value)<=h[g]){e=g;break}null==e&&(e=9);a=new ol.Feature({geometry:new ol.geom.Point(ol.proj.fromLonLat([b,d]))});a.setStyle(c[e]);f.push(a)}});return new ol.layer.Vector({source:new ol.source.Vector({features:f})})}
...
"inline");$("a#export_map_image").css("display","geography_point"==selected_type||geography_point_lookup_fields.includes(selected_field)?"inline":"none");$("#export").css("display","block")}function set_prefs(a,b){$.ajax(prefs_ajax_url+"&update=1&attribute="+a+"&value="+b)};
