...
load_geography_after_prefs_loaded(c+"&lookup_coordinates=1",b):load_pie(c,b,segments)});$('input[name="field_type"]').on("change",function(){var a=get_field_type();$("#field").empty();$.each({fields:field_list,loci:locus_list,schemes:scheme_list}[a],function(b,c){"schemes"==a?(b=c.label,c=c.field):b=c.replace(/^.+\.\./,"");jQuery("<option/>",{value:c,html:b}).appendTo("#field")});$("#field").change();fasta="loci"==a?1:0});position_controls();$(window).resize(function(){position_controls()});$("#export_image").off("click").click(function(){d3.select("#bb_chart").selectAll("path").attr("fill",
"none");d3.select("#bb_chart").selectAll("path.domain").attr("stroke","black");d3.select("#bb_chart").selectAll(".tick line").attr("stroke","black");d3.select("#bb_chart").selectAll(".bb-axis-y2").attr("display","none");d3.select("#bb_chart").selectAll(".bb-axis-x").attr("display","none");d3.select("#bb_chart").select(".bb-axis-x").attr("display","inline");d3.select("#map").selectAll("path").attr("fill","none");d3.select("#map").selectAll("path").attr("stroke","#444");d3.select("#map").selectAll(".background").attr("fill",
"none");var a=d3.select("svg").attr("xmlns","http://www.w3.org/2000/svg").node().parentNode.innerHTML;a=a.replace(/<\/svg>.*$/,"</svg>");a=new Blob([a],{type:"image/svg+xml"});var b=$("#field").val().replace(/^.+\.\./,"")+".svg";saveAs(a,b)});$("#expand_themes").click(function(){$("#themes_shown").toggle();$("#themes_hidden").toggle();$("#themes_hidden").is(":visible")?$("#expanded_themes").hide(500):$("#expanded_themes").show(500)})});
