...
"Conic Equal Area":d3.geoConicEqualArea,Equirectangular:d3.geoEquirectangular,"Natural Earth":d3.geoNaturalEarth,Mercator:d3.geoMercator,Robinson:d3.geoRobinson,Stereographic:d3.geoStereographic,Times:d3.geoTimes,"Transverse Mercator":d3.geoTransverseMercator,"Winkel tripel":d3.geoWinkel3},m=g[theme];d3.json(a).then(function(l){"country"==b&&(l=merge_terms(l));var k=get_range(l),n=d3.geomap.choropleth().geofile(h).colors(m).column("value").format(d3.format(",d")).legend({width:50,height:120}).projection(f[projection]).duration(1E3).domain([0,
k.recommended]).valueScale(d3.scaleQuantize).unitId(d).units(e).postUpdate(function(){finished_map(a,b)}),p=d3.select("#map").datum(l);n.draw(p);$(window).resize(function(){b===selected_field&&delay(function(){c!=$("#map").width()&&(c=$("#map").width(),$("#map").html(""),load_map(a,b))},500)});$("#colour_range").slider({min:0,max:4,value:k.options.indexOf(k.recommended),slide:function(a,b){n.domain([0,k.options[b.value]]).duration(0).update()}});$(".theme").off("click").click(function(){n.colors(g[this.id]).duration(0).update();
...
var delay=function(){var a=0;return function(b,c){clearTimeout(a);a=setTimeout(b,c)}}();function get_range(a){var b=a.length;b=parseInt((10<b?.05:.25)*b);for(var c=10,d,e,h=[],g;;){for(var f=[1,2,5],m=0;m<f.length;m++){d=f[m]*c;h.push(d);if(e&&5<=h.length){g=1;break}for(var l=4*d/5,k=0,n=0;n<a.length;n++)a[n].value>=l&&k++;k<=b&&!e&&(e=d)}if(g)break;c*=10;if(1E7<d){e||(e=d);break}}return{recommended:e,options:h.slice(-5)}}
...
function load_pie(a,b,c){$("#bar_controls").css("display","none");$("#line_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");if("undefined"!=typeof b){var d=b.replace(/^.+\.\./,"");d=d.replace(/^s_\d+_/,"");var e=d3.format(".1f");d3.json(a).then(function(h){for(var g=pie_json_to_cols(h,c),f=pie_json_to_cols(h,50),m=0,l=0;l<f.length;l++)"Others"==f.columns[l][0]&&(m=1);m||f.columns.push(["Others",0]);d+=" ("+g.count+" value"+(1==
g.count?"":"s")+")";var k=bb.generate({bindto:"#bb_chart",title:{text:d},data:{columns:f.columns,type:pie?"pie":"donut",order:null,colors:{Others:"#aaa"}},pie:{label:{show:!0},expand:!1},legend:{show:!0,position:"bottom"},size:{height:500},tooltip:{format:{value:function(a,b,c){return a+" ("+e(100*b)+"%)"}}}});k.unload();k.load({columns:g.columns});$("#segments").on("slidechange",function(){var a=$("#segments").slider("value");$("#segments_display").text(a);segments!=a&&set_prefs("segments",a);segments=
a;a=pie_json_to_cols(h,segments);k.unload();k.load({columns:a.columns})});c!=segments&&(g=pie_json_to_cols(h,segments),k.unload(),k.load({columns:g.columns}));$(".transform_to_donut").off("click").click(function(){$(".transform_to_donut").css("display","none");$(".transform_to_pie").css("display","inline");pie=0;set_prefs("pie",0);k.unload();load_pie(a,b,c)});$(".transform_to_pie").off("click").click(function(){$(".transform_to_pie").css("display","none");$(".transform_to_donut").css("display","inline");
pie=1;set_prefs("pie",1);k.unload();load_pie(a,b,c)});$(".transform_to_map").off("click").click(function(){$(".transform_to_map").css("display","none");load_map(a,b);$(".transform_to_pie").css("display","inline");$(".transform_to_donut").css("display","inline");pie=1;set_prefs("pie",1)});$("#segment_control").css("display","block");$("#segments").slider({min:5,max:50,value:segments});$("#segments_display").text(segments);$(".transform_to_map").css("display",map_fields.includes(b)?"inline":"none");
$(".transform_to_pie").css("display",pie?"none":"inline");$(".transform_to_donut").css("display",pie?"inline":"none");$(".transform_to_bar").css("display","none");$(".transform_to_line").css("display","none");$("#pie_controls").css("display","block");show_export_options();$("div#waiting").css("display","none")},function(a){console.log(a);$("#bb_chart").html('<p style="text-align:center;margin-top:5em"><span class="error_message">Error accessing data.</span></p>')})}}
function pie_json_to_cols(a,b){var c=[],d=[],e=0,h=0,g=0;a.forEach(function(a){a.label=a.label.toString();""==a.label&&(a.label="No value");e++;e>=b?(h+=a.value,g++,e==b&&(d=[a.label,a.value])):c.push([a.label,a.value])});1==g?c.push(d):1<g&&c.push(["Others",h]);return{columns:c,count:e}}
function load_line(a,b,c){$("#bar_controls").css("display","none");$("#pie_controls").css("display","none");$("#map_controls").css("display","none");$("#geography_controls").css("display","none");$(".transform_to_map").css("display","none");$("#line_height").off("slidechange");var d=b.replace(/^.+\.\./,"");d3.json(a).then(function(e){remove_null(e);var h=["value"],g=["date"],f=0,m=0;e.forEach(function(a){f++;g.push(a.label);c?(m+=a.value,h.push(m)):h.push(a.value)});d+=" ("+f+" value"+(1==f?"":"s")+
")";var l=bb.generate({bindto:"#bb_chart",title:{text:d},data:{x:"date",columns:[g,h],type:line?"line":"bar",order:"asc"},axis:{x:{type:"timeseries",tick:{format:"%Y-%m-%d",count:100,rotate:90,fit:!0},height:100}},legend:{show:!1}});l.resize({height:height});$(".transform_to_bar").off("click").click(function(){l.unload();load_line(a,b,0);$(".transform_to_bar").css("display","none");$(".transform_to_line").css("display","inline");line=0});$(".transform_to_line").off("click").click(function(){l.unload();
...
function load_geography(a,b){$("#bb_chart").html("");$("#geography").css("height","400px");var c=["RoadOnDemand","AerialWithLabelsOnDemand"],d=["Map","Aerial"],e=[];if(bingmaps_api){var h=$("input[name='geography_view']:checked").val(),g;var f=0;for(g=c.length;f<g;++f)e.push(new ol.layer.Tile({visible:d[f]==h?!0:!1,preload:Infinity,source:new ol.source.BingMaps({key:bingmaps_api,imagerySet:c[f]})}))}else e.push(new ol.layer.Tile({source:new ol.source.OSM({crossOrigin:null})}));d3.json(a).then(function(a){var c=
