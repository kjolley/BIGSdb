		...
		foreach my $required ( '1', '0' ) {
			foreach my $field ( @{ $self->{'xmlHandler'}->get_field_list } ) {
					...
					|| ( !$required_field && !$required ) )
				{
					...
					my $bad_field =
					  $self->is_field_bad( $self->{'system'}->{'view'},
						$field, $newdata{$field} );
					if ($bad_field) {
		...
		foreach (@$loci) {
			if ( $q->param("locus:$_") ) {
					...
					&& !BIGSdb::Utils::is_int( $newdata{"locus:$_"} ) )
				{
					...
					push @bad_field_buffer, "Locus '$_' allele value is invalid - it must match the regular expression /$locus_info->{'allele_id_regex'}/";
				}
			}
		}
		if (@bad_field_buffer) {
...
"INSERT INTO allele_designations (isolate_id,locus,allele_id,sender,status,method,curator,date_entered,datestamp) VALUES ('$newdata{'id'}','$_','$newdata{\"locus:$_\"}','$newdata{'sender'}','$newdata{\"locus:$_\_status\"}','manual','$newdata{'curator'}','today','today')";
				...
				foreach my $new (@new_aliases){	
					...
					push @inserts,"INSERT INTO isolate_aliases (isolate_id,alias,curator,datestamp) VALUES ($newdata{'id'},'$clean_new',$newdata{'curator'},'today')";
				}
...
"<div class=\"box\" id=\"statusbad\"><p>PubMed ids must be integers.</p></div>\n";
					...
					push @inserts,"INSERT INTO refs (isolate_id,pubmed_id,curator,datestamp) VALUES ($newdata{'id'},'$clean_new',$newdata{'curator'},'today')";
				}				
				if ($insert){
					$" = ';';
...
"<div class=\"box\" id=\"queryform\"><p>Please fill in the fields below - required fields are marked with an exclamation mark (!).</p>\n";
	...
	my @users;
	my %usernames;
	while ( my ( $userid, $username, $firstname, $surname ) =
		$sql->fetchrow_array )
	{
		push @users, $userid;
	...
	my $max_loci =
	  scalar @{ $self->{'xmlHandler'}->get_field_list } +10
	  ; #Number of loci over which they will be displayed at the bottom of table, rather than in new column
		...
		foreach my $field ( @{ $self->{'xmlHandler'}->get_field_list } ) {
				...
				|| ( !$required_field && !$required ) )
			{
				...
				if ( $thisfield{'optlist'} ) {
					my @optlist;
					foreach my $option (
						$self->{'xmlHandler'}->get_field_option_list($field) )
					{
						push @optlist, $option;
					}
					print $q->popup_menu(
	...
	if (scalar @$loci <= 100){
		foreach (@$loci) {
			...
			$cleaned =~ tr/_/ /;
			$locus_buffer .=
			  "<tr><td style=\"text-align:right\">$cleaned: </td><td style=\"white-space: nowrap\">";
			$locus_buffer .= $q->textfield(
				-name    => "locus:$_",
				-size    => 10,
				-default => $newdata{$_},
				-tabindex => $tabindex
		...
		$locus_buffer.="<tr><td style=\"text-align:right\">Loci: </td><td style=\"white-space: nowrap\">Too many to display.  You can batch add allele designations after entering isolate provenace data.</td></tr>\n";
	}
	...
	if ( !$q->param('submit') ) {
		print "<tr><td>";
		print
"<a href=\"$self->{'system'}->{'script_name'}?db=$self->{'instance'}&amp;page=isolateAdd\" class=\"resetbutton\">Reset</a>";
