...
sub print_content {
	...
	my $show_oops = 1;
	$show_oops = 0 if  $self->{'error'} eq 'userNotAuthenticated' || $self->{'error'} eq 'accessDisabled';
...
"<p>Unknown function '$function' requested - either an incorrect link brought you here or this functionality has not been implemented yet!</p>";
		...
		print
"<p>Invalid (or no) database description file specified!</p>";	
...
"<p>Invalid database type specified! Please set dbtype to either 'isolates' or 'sequences' in the system attributes of the XML description file for this database.</p>";	
		...
		print
"<p>You are attempting to access this database from an invalid script path.</p>";	

	} elsif ($self->{'error'} eq 'invalidCurator'){
		print
"<p>You are not a curator for this database.</p>";	
	} elsif ($self->{'error'} eq 'noConnect'){
		print
"<p>Can not connect to database!</p>";	
	} elsif ($self->{'error'} eq 'noAuth'){
		print
"<p>Can not connect to the authentication database!</p>";	
	} elsif ($self->{'error'} eq 'noPrefs'){
		if ($self->{'fatal'}){
			print
"<p>The preference database can be reached but it appears to be misconfigured!</p>";
		} else {
			print
"<p>Can not connect to the preference database!</p>";
		}	
	} elsif ($self->{'error'} eq 'userAuthenticationFiles'){
		print
"<p>Can not open the user authentication database!</p>";
...
"<p>No authentication mechanism has been set in the database configuration!</p>";
		...
		print
"<p>An unforeseen error has occurred - please contact the system administrator.</p>";
