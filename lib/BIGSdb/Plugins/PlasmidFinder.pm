...
sub run_job {
	...
	foreach my $isolate_id (@$isolate_ids) {
		...
		eval {
			...
			if ( $results->{'seq_regions'} ) {
				my $regions = $results->{'seq_regions'};
				foreach my $key (
					sort {
							 $regions->{$a}->{'query_id'} <=> $regions->{$b}->{'query_id'}
		...
		if ($table_rows) {
			local $" = q(</th><th>);
			my @header_fields = ( 'id', $labelfield );
			push @header_fields, ( $renamed_fields{$_} // $_ ) foreach @fields;
			$table_html = qq(<div class="scrollable"><table class="resultstable"><tr><th>@header_fields</th></tr>)
			  . qq($table_rows</table></div>);
		}
		$self->{'jobManager'}
		...
		if ( @$isolate_ids == 1 ) {
			if ( -e $json_path ) {
				...
				$html = q(<p class="statusbad">No output generated by PlasmidFinder.</p>);
