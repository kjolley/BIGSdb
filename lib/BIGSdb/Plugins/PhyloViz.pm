...
sub run {
	...
	if ( !@$selected_isolates_fields ) {
	    say q(<div class="box" id="statusbad"><p>You must at least select <strong>one isolate field!</strong></p></div>);
	    return;
	}

	# Get selected schemes
	######################
	my $schemes_id = $self->{'datastore'}->run_query("SELECT id, description FROM schemes ORDER BY id ASC",
							 undef,
							 { 'fetch' => 'all_arrayref' });
	...
	if( scalar(@$schemes_id) ){
		...
		if( $q->param("s_${scheme_id}") and $q->param("s_${scheme_id}") == 1){
		    ...
		    $selected_schemes->{$scheme_id}->{'scheme_fields'} = $self->{'datastore'}->get_scheme_fields($scheme_id);
		}
	    }
	}
	...
	if ( !@$selected_loci ) {
	    ...
	    if( $self->generate_profile_file({'file' => $profile_file, 'schemes' => $selected_schemes,
					      'isolates' => $isolates_ids, 'loci' => $selected_loci})){
		say q(<div class="box" id="statusbad"><p>Nothing found in the database for your isolates!</p></div>);
		return;
	    }
	    $self->generate_auxiliary_file({'file' => $auxiliary_file, 'schemes' => $selected_schemes,
					    'isolates' => $isolates_ids, 'fields' => $selected_isolates_fields});
	    # Upload data files to phyloviz online using python script
	    my ($phylo_id, $msg) = $self->upload_data_to_phyloviz({'profile'   => $profile_file,
