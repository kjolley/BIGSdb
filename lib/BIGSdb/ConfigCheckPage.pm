...
sub print_content {
	...
	print "<div class=\"scrollable\"><table class=\"resultstable\"><tr><th>Program</th><th>Path</th><th>Installed</th><th>Executable</th></tr>\n";
	my %helpers = (
	...
	foreach ( sort {$a cmp $b} keys  %helpers ) {
		print "<tr class=\"td$td\"><td>$_</td><td>$helpers{$_}</td><td>"
		  . ( -e ( $helpers{$_} ) ? '<span class="statusgood">ok</span>'  : '<span class="statusbad">X</span>' ) . "</td><td>"
	...
	if ($self->{'system'}->{'dbtype'} eq 'isolates'){
		...
		foreach (@$loci){
			...
			my $seq;
			eval {
				$seq = $self->{'datastore'}->get_locus($_)->get_allele_sequence('1');
			};
		...
		foreach (@$schemes){
			...
			eval {
				$self->{'datastore'}->get_scheme($_)->get_field_values_by_profile(\@values);
			};
			if ($@){
				$buffer .= '<span class="statusbad">X</span>';
			} else {
		...
		if ($buffer){
			print "<div class=\"scrollable\"><table class=\"resultstable\"><tr><th>Scheme description</th><th>Database</th><th>Host</th><th>Port</th>
		...
		foreach (@$client_dbs){
			...
			$buffer.="<tr class=\"td$td\"><td>$client_info->{'name'}</td><td>$client_info->{'description'}</td><td>$client_info->{'dbase_name'}</td><td>" . ($client_info->{'dbase_host'} || 'localhost') . "</td>
		...
		if ($buffer){
			...
			print "<div class=\"scrollable\"><table class=\"resultstable\"><tr><th>Name</th><th>Description</th><th>Database</th><th>Host</th><th>Port</th><th>Database accessible</th></tr>";
			print $buffer;
			print "</table></div>\n";
		}
	}
