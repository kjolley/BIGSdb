...
sub print_content {
	...
	my $exists =
	  $self->{'datastore'}
	  ->run_simple_query( "SELECT COUNT(*) FROM composite_fields WHERE id=?",
		$id )->[0];
	if ( !$exists ) {
		print
"<div class=\"box\" id=\"statusbad\">Composite field '$id' has not been defined.</div>\n";
		return;
	}
	...
	if ($q->param('update')){
		...
		if (!$self->{'xmlHandler'}->is_field($position_after)){
			...
			$self->{'db'}->do("UPDATE composite_fields SET position_after='$position_after',main_display='$main_display',curator=$curator_id,datestamp='today'");
			$self->{'db'}->commit();
		}
	}
	
	print $q->start_form;
	my $field_info = $self->{'datastore'}->run_simple_query("SELECT position_after,main_display,curator,datestamp FROM composite_fields WHERE id=?",$id);
...
"<tr class=\"td$td\"><th>field</th><th>empty value</th><th>regex</th><th>curator</th><th>datestamp</th><th>delete</th><th>edit</th><th>move</th></tr>\n";
	my $qry =
"SELECT * FROM composite_field_values WHERE composite_field_id = '$id' ORDER BY field_order";
	...
	my $highest = $self->{'datastore'}->run_simple_query(
"SELECT max(field_order) FROM composite_field_values WHERE composite_field_id=?",
		$id
	)->[0];
...
"UPDATE composite_field_values SET field_order = 0 WHERE field_order = $field_order AND composite_field_id='$id'"
					...
					$self->{'db'}->do(
"UPDATE composite_field_values SET field_order = $field_order WHERE field_order = "
						  . ( $field_order - 1 )
						  . " AND composite_field_id='$id'" );
					$self->{'db'}
					  ->do( "UPDATE composite_field_values SET field_order = "
						  . ( $field_order - 1 )
				...
				if ($@) {
					$logger->error(
						"Can't update composite_field_values order $@");
					$self->{'db'}->rollback;
				} else {
					$self->{'db'}->commit;
				}
...
"UPDATE composite_field_values SET field_order = $field_order WHERE field_order = "
			...
			eval {
				$self->{'db'}->do(
"DELETE FROM composite_field_values WHERE field_order=$field_order AND composite_field_id='$id'"
				);
			};
			if ($@) {
				...
				$logger->info(
"Deleted composite_vield_value field_order:$field_order for composite_field:$id"
				);
				$self->{'db'}->commit;

				#close up gaps in field_order numbers
				my $max = $self->{'datastore'}->run_simple_query(
"SELECT MAX(field_order) FROM composite_field_values WHERE composite_field_id=?",
					$id
				)->[0];
				my $qry =
"SELECT COUNT(*) FROM composite_field_values WHERE composite_field_id=? AND field_order=?";
				my $sql = $self->{'db'}->prepare($qry);
				$qry =
"SELECT field_order FROM composite_field_values WHERE composite_field_id=? AND field_order>? ORDER BY field_order";
...
"UPDATE composite_field_values SET field_order=$next WHERE composite_field_id='$id' AND field_order=$old_order"
			...
			my $text_field;
			$edit_buffer .=
"<h2>Edit:</h2><table><tr><td style=\"text-align:right\">Field: </td><td>";
			if ( $field =~ /^f_(.+)/ ) {
				my $field_value      = $1;
				...
				if ($is_isolate_field) {
					...
					$edit_buffer .=
"<span class=\"statusbad\">$field_value (INVALID FIELD)</span>\n";
					$invalid = 1;
				}
			} elsif ( $field =~ /^t_(.+)/ ) {
				my $field_value = $1;
				$edit_buffer .= $q->textfield(
					-name    => 'field_value',
					-default => $field_value
				);
				$text_field = 1;
				...
				if ($is_locus) {
					...
					$edit_buffer .=
"<span class=\"statusbad\">$field_value (INVALID LOCUS)</span>\n";
					$invalid = 1;
				}
			} elsif ( $field =~ /^s_(\d+)_(.+)/ ) {
				my $scheme_id       = $1;
				...
				my $scheme_list =
				  $self->{'datastore'}
				  ->run_list_query("SELECT id FROM schemes ORDER BY display_order,id");
				foreach my $s_id (@$scheme_list) {
					foreach (
						@{ $self->{'datastore'}->get_scheme_fields($s_id) } )
					{
						...
						my $scheme_info =
						  $self->{'datastore'}->get_scheme_info($s_id);
						my $cleaned_field = $_;
						$cleaned_field =~ tr/_/ /;
						$cleaned{"s_$s_id\_$_"} =
						  "$cleaned_field ($scheme_info->{'description'})";
					}
				}
				if ($is_scheme_field) {
					$edit_buffer .= $q->popup_menu(
					...
					$edit_buffer .=
"<span class=\"statusbad\">$field_value (INVALID SCHEME FIELD)</span>\n";
					$invalid = 1;
				}
			}
			if ( !$invalid ) {
...
"</td></tr>\n<tr><td style=\"text-align:right\">Empty value: </td><td>";
				...
				$edit_buffer .= "</td></tr>\n";
				$edit_buffer .=
"<tr><td style=\"text-align:right\">Regex: </td><td class=\"code\" colspan=\"2\">";
				if ($text_field) {
					$edit_buffer .= $q->textfield(
						-name     => 'regex',
						-size     => 50,
				...
				$edit_buffer .= "</td></tr>\n";
				my $curator =
				  $self->{'datastore'}->get_user_info( $data->{'curator'} );
				$edit_buffer .=
"<tr><td style=\"text-align:right\"><b>Curator: </b></td><td><b>"
...
"UPDATE composite_field_values SET field='$field_value',empty_value='$empty_value',regex='$regex',curator='$curator_id',datestamp='today' WHERE field_order=$field_order AND composite_field_id='$id'"
		...
		|| $q->param('new_locus')
		|| $q->param('new_scheme_field') || $q->param('new_isolate_field'))
	{
		my $nextref = $self->{'datastore'}->run_simple_query(
"SELECT MAX(field_order) FROM composite_field_values WHERE composite_field_id=?",
			$id
		);
		my ($next) = @$nextref;
...
"INSERT INTO composite_field_values (composite_field_id,field_order,field,empty_value,regex,curator,datestamp) VALUES ('$id',$next,'$field_value',null,null,$curator,'today')"
			...
			if ($@) {
				...
				$logger->info(
"Added new composite_field_value for $id: field:$field_value; field_order:$next."
				);
				$self->{'db'}->commit;
			}
		}
	...
	while ( my $data = $sql->fetchrow_hashref() ) {
		...
		if ( $data->{'field'} =~ /^f_(.+)/ ) {
			$field =
"<span class=\"field\">$1</span> <span class=\"comment\">[isolate field]</span>";
			$missing = "<span class=\"field\">$data->{'empty_value'}</span>";
		} elsif ( $data->{'field'} =~ /^l_(.+)/ ) {
			my $locus = $1;
			$field =
"<span class=\"locus\">$locus</span> <span class=\"comment\">[locus]</span>";
			my $locus_info = $self->{'datastore'}->get_locus_info($locus);
			$missing = "<span class=\"locus\">$data->{'empty_value'}</span>";
			...
			my $scheme_field_info =
			  $self->{'datastore'}
			  ->get_scheme_field_info( $scheme_id, $field_value );
			$field = "<span class=\"scheme\">$field_value</span>";
			$field .= " <span class=\"comment\">[$desc field]</span>" if $desc;
			if ( ref $scheme_field_info ne 'HASH' ) {
				$field .=
				  " <span class=\"statusbad\">(INVALID SCHEME FIELD)</span>\n";
			}
			$missing = "<span class=\"scheme\">$data->{'empty_value'}</span>";
		} elsif ( $data->{'field'} =~ /^t_(.+)/ ) {
		...
		print
"<tr class=\"td$td\"><td>$field</td><td>$data->{'empty_value'}</td><td class=\"code\">$data->{'regex'}</td>";
		print
"<td>$curator->{'first_name'} $curator->{'surname'}</td><td>$data->{'datestamp'}</td><td>";
		print $q->submit(
			-name  => "$data->{'field_order'}_delete",
			-label => 'delete',
			-class => 'smallbutton'
		);
		print "</td><td>";
		print $q->submit(
			-name  => "$data->{'field_order'}_edit",
			-label => 'edit',
			-class => 'smallbutton'
		);
		print "</td><td>";
		print $q->submit(
			-name  => "$data->{'field_order'}_up",
			-label => 'up',
			-class => 'smallbutton'
		);
		print $q->submit(
			-name  => "$data->{'field_order'}_down",
			-label => 'down',
			-class => 'smallbutton'
		);
		print "</td>";
	...
	if ( !$edit_buffer ) {
		...
		$add_buffer .= "</td><td>";
		$add_buffer .= $q->submit(
			-name  => 'new_isolate_field',
			-label => 'Add new isolate field',
			-class => 'smallbutton'
		);
		$add_buffer .= "</td></tr>\n<tr><td style=\"text-align:right\">";
		...
		$add_buffer .= $q->popup_menu(
			-name   => 'new_locus_value',
			-values => [@locus_list]
		);
		$add_buffer .= "</td><td>";
		$add_buffer .= $q->submit(
			-name  => 'new_locus',
			-label => 'Add new locus field',
			-class => 'smallbutton'
		);
		$add_buffer .= "</td></tr>\n<tr><td style=\"text-align:right\">";
		my @scheme_field_list = '';
		...
		foreach my $scheme_id (@$scheme_list) {
			foreach ( @{ $self->{'datastore'}->get_scheme_fields($scheme_id) } )
			{
				...
				my $scheme_info =
				  $self->{'datastore'}->get_scheme_info($scheme_id);
				my $cleaned_field = $_;
				$cleaned_field =~ tr/_/ /;
				$cleaned{"$scheme_id\_$_"} =
				  "$cleaned_field ($scheme_info->{'description'})";
			}
		}
		$add_buffer .= $q->popup_menu(
		...
		$add_buffer .= "</td><td>";
		$add_buffer .= $q->submit(
			-name  => 'new_scheme_field',
			-label => 'Add new scheme field',
			-class => 'smallbutton'
		);
