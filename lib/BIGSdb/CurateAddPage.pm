...
sub print_content {
	...
	if ( !$self->can_modify_table($table) ) {
		if ($table eq 'sequences' && $q->param('locus') || $table eq 'locus_descriptions'){
			...
			print "<div class=\"box\" id=\"statusbad\"><p>Your user account is not allowed to add $locus $record_type"."s to the database.</p></div>\n";
		} else {
		...
		if (!$self->is_admin){
			...
			if (!$allowed){
				print "<div class=\"box\" id=\"statusbad\"><p>Your user account is not allowed to add ". ($table eq 'sequence_refs' ? 'references' : 'accession numbers') ." for this locus.</p></div>\n";
				return;					
			}
		}
...
this sequence then make sure that the 'Override sequence similarity check' box is ticked.<br />";
			...
			my $sql = $self->{'db'}->prepare("SELECT field,required,value_format,value_regex,option_list FROM locus_extended_attributes WHERE locus=?");
			eval {
				$sql->execute( $newdata{'locus'});
			};
			if ($@){
			...
			while (my ($field,$required,$format,$regex,$option_list) = $sql->fetchrow_array){
				...
				if ($required && $newdata{$field} eq ''){
					...
					$field  =~ s/'/\\'/g;
					if ($newdata{$field} ne ''){
						my $insert = "INSERT INTO sequence_extended_attributes(locus,field,allele_id,value,datestamp,curator) VALUES ('$cleaned_locus','$field','$newdata{'allele_id'}','$newdata{$field}','now',$newdata{'curator'})";
			...
			if (@missing_field){
				$"=', ';
				push @problems,"Please fill in all extended attribute fields.  The following extended attribute fields are missing: @missing_field";
			}
			...
			foreach my $new (@new_pubmeds){	
				...
				if (!BIGSdb::Utils::is_int($new)){
					...
					(my $cleaned_locus =$newdata{'locus'})  =~ s/'/\\'/g;
					push @extra_inserts,"INSERT INTO sequence_refs (locus,allele_id,pubmed_id,curator,datestamp) VALUES ('$cleaned_locus','$newdata{'allele_id'}',$new,$newdata{'curator'},'today')";
				}
			}
			...
			foreach my $databank (@databanks){
				...
				foreach my $new (@new_accessions){	
					...
					(my $cleaned_locus =$newdata{'locus'})  =~ s/'/\\'/g;
					push @extra_inserts,"INSERT INTO accession (locus,allele_id,databank,databank_id,curator,datestamp) VALUES ('$cleaned_locus','$newdata{'allele_id'}','$databank','$clean_new',$newdata{'curator'},'today')";
				}				
			}	
			...
			foreach my $new (@new_aliases){	
				chomp $new;
				next if $new eq '';					
				push @extra_inserts,"INSERT INTO locus_aliases (locus,alias,curator,datestamp) VALUES ('$cleaned_locus','$new',$newdata{'curator'},'today')";
			}
			my @new_pubmeds = split/\r?\n/,$q->param('pubmed');
			foreach my $new (@new_pubmeds){	
				chomp $new;
				...
				if (!BIGSdb::Utils::is_int($new)){
					...
					push @extra_inserts,"INSERT INTO locus_refs (locus,pubmed_id,curator,datestamp) VALUES ('$cleaned_locus',$new,$newdata{'curator'},'today')";
				}
			}
			...
			foreach my $new (@new_links){	
				...
				if ($new !~ /^(.+?)\|(.+)$/){
					...
					$url =~ s/'/\\'/g;
					$desc =~ s/'/\\'/g;
					push @extra_inserts,"INSERT INTO locus_links (locus,url,description,link_order,curator,datestamp) VALUES ('$cleaned_locus','$url','$desc',$i,$newdata{'curator'},'today')";
				}
				$i++;
			}
			...
			if ($q->param('experiment')){
				...
				my $insert = "INSERT INTO experiment_sequences (experiment_id,seqbin_id,curator,datestamp) VALUES ($experiment,$newdata{'id'},$newdata{'curator'},'now')";
				push @extra_inserts, $insert;
			}
		}
		if ($table eq 'sequences'){
		...
		#special case to check that profile exists when adding PubMed number
			...
			if ($newdata{'id'} =~ /^\d/){
				push @problems, "Locus names can not start with a digit.  Try prepending an underscore (_) which will get hidden in the query interface.";
			}
			if ($newdata{'id'} =~ /\./){
...
"<p>Data entry would have resulted in records with either duplicate ids or another unique field with duplicate values.</p>\n";
				...
				if ( $table eq 'composite_fields' ) {
					print
"<p><a href=\"?db=$instance&amp;page=compositeUpdate&amp;id=$newdata{'id'}\">Add values and fully customize this composite field</a>.</p>";
				}
				my $locus_clause;
				if ( $table eq 'sequences' ) {
...
sub get_javascript {
	my ($self) = @_;
	my $q = $self->{'cgi'};
	return if $q->param('table') ne 'sequences';
	my $buffer = << "END";
\$(function () {
...
sub next_id {
	...
	if (!$self->{'sql'}->{'next_id'}->{$table}){
		...
		my $qry = "SELECT l.id + 1 AS start FROM $table AS l left outer join $table AS r on l.id+1=r.id where r.id is null ORDER BY l.id LIMIT 1";
...
sub _next_id_profiles {
	...
	if (!$self->{'sql'}->{'next_id_profiles'}){
		my $qry = "SELECT DISTINCT CAST(profile_id AS int) FROM profiles WHERE scheme_id = ? AND CAST(profile_id AS int)>0 ORDER BY CAST(profile_id AS int)";
...
sub id_exists {
	my ( $self, $id ) = @_;
	my $num = $self->{'datastore'}->run_simple_query( "SELECT COUNT(*) FROM $self->{'system'}->{'view'} WHERE id=?", $id );
	return $num->[0];
}
