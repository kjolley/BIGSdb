...
sub _print_scheme_table {
	...
	my $curator_sql = $self->{'db'}->prepare("SELECT curator_id FROM locus_curators WHERE locus=? AND (hide_public IS NULL OR NOT hide_public)");
	my ($scheme_descs_exist,$scheme_aliases_exist,$scheme_curators_exist);
	if ($scheme_id){
		$scheme_descs_exist = $self->{'datastore'}->run_simple_query("SELECT COUNT(*) FROM locus_descriptions LEFT JOIN scheme_members ON locus_descriptions.locus=scheme_members.locus WHERE scheme_id=?",$scheme_id)->[0];
		$scheme_aliases_exist = $self->{'datastore'}->run_simple_query("SELECT COUNT(*) FROM locus_aliases LEFT JOIN scheme_members ON locus_aliases.locus=scheme_members.locus WHERE scheme_id=?",$scheme_id)->[0];
		$scheme_curators_exist = $self->{'datastore'}->run_simple_query("SELECT COUNT(*) FROM locus_curators LEFT JOIN scheme_members ON locus_curators.locus=scheme_members.locus WHERE scheme_id=? AND (hide_public IS NULL OR NOT hide_public)",$scheme_id)->[0];
	} else {
		$scheme_descs_exist = $self->{'datastore'}->run_simple_query("SELECT COUNT(*) FROM locus_descriptions LEFT JOIN scheme_members ON locus_descriptions.locus=scheme_members.locus WHERE scheme_id IS NULL")->[0];
		$scheme_aliases_exist = $self->{'datastore'}->run_simple_query("SELECT COUNT(*) FROM locus_aliases LEFT JOIN scheme_members ON locus_aliases.locus=scheme_members.locus WHERE scheme_id IS NULL")->[0];	
		$scheme_curators_exist = $self->{'datastore'}->run_simple_query("SELECT COUNT(*) FROM locus_curators LEFT JOIN scheme_members ON locus_curators.locus=scheme_members.locus WHERE scheme_id IS NULL AND (hide_public IS NULL OR NOT hide_public)")->[0];	
	}
	if (@$loci) {
		$desc =~ s/\&/\&amp;/g;
		print "<h2>$desc</h2>\n";
		...
		foreach (@$loci) {
			...
			if ($@) {
				$logger->error("Can't count sequences $@");
			}
...
"<a href=\"$self->{'system'}->{'script_name'}?db=$self->{'instance'}&amp;page=downloadAlleles&amp;locus=$_\" class=\"downloadbutton\">&darr;</a>" if $count;
			...
			if ($scheme_curators_exist){
				...
				foreach my $curator (@curators){
					print ', ' if !$first;
					my $first_initial = substr($info->{$curator}->{'first_name'},0,1) .'. ' if $info->{$curator}->{'first_name'}; 
...
sub _print_sequences {
	...
	my $qry        =
	  "SELECT allele_id,sequence FROM sequences WHERE locus=? ORDER BY "
