...
sub print_content {
	...
	print "</ul>\n";
	$desc->{'description'} =~ s/\n/<br \/>/g;
	print "<p>$desc->{'description'}</p>" if $desc->{'description'};
	...
	my $links = $self->{'datastore'}->run_list_query_hashref("SELECT url,description FROM locus_links WHERE locus=? ORDER BY link_order",$locus);
	if (@$links){
		print "<h2>Links</h2>\n<ul>\n";
		foreach (@$links){
			$_->{'url'} =~ s/\&/&amp;/g;			
