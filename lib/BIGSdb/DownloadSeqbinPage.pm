...
#the Free Software Foundation, #Written by Keith Jolley
#Copyright (c) 2010, University of Oxford
#E-mail: keith.jolley@zoo.ox.ac.uk
#
#This file is part of Bacterial Isolate Genome Sequence Database (BIGSdb).
...
sub break_line {
	#pass by reference as sequences may be long
	my ( $string_ref, $length ) = @_;
	my $seq;
	my $pos = 1;
	for ( my $i = 0 ; $i < length($$string_ref) ; $i++ ) {
		$seq .= substr $$string_ref, $i, 1;
		if ( $pos == $length ) {
			$seq .= "\n" if $i != length($$string_ref) - 1;
			$pos = 0;
		}
		$pos++;
	}
