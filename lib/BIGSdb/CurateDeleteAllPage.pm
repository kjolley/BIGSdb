...
sub print_content {
		...
		&& !( $table eq 'samples' && @{ $self->{'xmlHandler'}->get_sample_field_list } ) )
	{
		...
		say
qq(<div class="box" id="statusbad"><p>Your user account is not allowed to delete records from the $table table.</p></div>);
		return;
	...
	{
		say
		  qq(<div class="box" id="statusbad"><p>Only administrators can batch delete from the $table table.</p></div>);
		return;
	}
		...
		foreach my $scheme_id (@$schemes) {
			if ( $query =~ /temp_scheme_$scheme_id\s/ ) {
				try {
					...
					say
qq(<div class="box" id="statusbad"><p>Can't copy data into temporary table - please check scheme configuration )
...
sub _delete {
	...
	if ( $table eq 'loci' && $delete_qry =~ /JOIN scheme_members/ && $delete_qry =~ /scheme_id is null/ ) {
		...
		&& (   $delete_qry =~ /JOIN sequence_flags/
			|| $delete_qry =~ /JOIN sequence_bin/
			|| $delete_qry =~ /JOIN scheme_members/ )
	  )
	{
		...
		$delete_qry =~
		  s/SELECT \*/SELECT allele_designations.isolate_id,allele_designations.locus,allele_designations.allele_id/;
	...
	if ($schemes_affected) {
		say
qq(<div class="box" id="statusbad"><p>Deleting these loci would affect scheme definitions - can not delete!</p></div>);
		return;
	}
	eval {
	...
	if ($@) {
		say
qq(<div class="box" id="statusbad"><p>Delete failed - transaction cancelled - no records have been touched.</p>);
		if ( $@ =~ /foreign key/ ) {
			...
			say
"<p>This is a bug in the software or database structure.  Please report this to the database administrator. "
		...
		if ( $table eq 'allele_designations' && @allele_designations ) {
			...
			my $seqtag_sql =
			  $self->{'db'}->prepare(
"DELETE FROM allele_sequences WHERE seqbin_id IN (SELECT id FROM sequence_bin WHERE isolate_id=?) AND locus=?"
			  );
