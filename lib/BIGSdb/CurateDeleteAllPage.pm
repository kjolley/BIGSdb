...
sub print_content {
	...
	if ( !$self->{'datastore'}->is_table($table) ) {
		...
		print
		  "<div class=\"box\" id=\"statusbad\"><p>Only administrators can batch delete from the $table table.</p></div>\n";
		return;		
	}
	...
	if ( $q->param('deleteAll') ) {
			...
			&& !$self->is_admin )
		{
			if ( $table eq $self->{'system'}->{'view'} ) {
				...
				$delete_qry =~
			s/FROM $table/FROM isolates WHERE id IN (SELECT id FROM $self->{'system'}->{'view'})/;
			} elsif ( $table eq 'allele_designations' || $table eq 'sequence_bin' || $table eq 'isolate_aliases' ) {
				$delete_qry =~ s/WHERE/AND/;
				$delete_qry =~
			s/FROM $table/FROM $table WHERE isolate_id IN (SELECT id FROM $self->{'system'}->{'view'})/;
...
s/FROM $table/FROM $table LEFT JOIN sequence_bin ON $table.seqbin_id=sequence_bin.id WHERE isolate_id IN (SELECT id FROM $self->{'system'}->{'view'})/;
		...
		print $q->start_form;
		$q->param( 'deleteAll', 1 );
		foreach (qw (page db query deleteAll table)) {
			print $q->hidden($_);
		}
